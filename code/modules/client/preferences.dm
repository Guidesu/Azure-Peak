...
/datum/preferences
	...
	var/character_sheet_tile_layout = "{}"
	...
	// Fated remains implemented for legacy saves, but its random-stat/second-
	// virtue path is no longer part of character creation. TAT owns those picks.
	...
	var/static/datum/patron/default_patron = /datum/patron/tribunal/custodius
	...
	// Ratwood chastity_collar port, Stage 1.
	/// Opt-in: whether this character can be fitted with / wear chastity devices and receive their flavor text at all.
	var/chastenable = FALSE
	/// Opt-in: whether this character can wear/be fitted with the most extreme chastity content (spiked devices).
	var/extreme_erp = FALSE
	/// 0 (CHASTITY_HARDMODE_DISABLED, default) or 1 (CHASTITY_HARDMODE_ENABLED) — see
	/// modular_dreamvalley/ported/ratwood/chastity_collar/_defines/chastity.dm and
	/// /obj/item/chastity/proc/is_hardmode_active(). Left as a literal here since this file compiles
	/// before modular_dreamvalley/_dreamvalley.dm in roguetown.dme, so the CHASTITY_HARDMODE_* defines
	/// aren't visible yet at this point in the compile.
	var/chastity_hardmode = 0
...
/datum/preferences/proc/ShowChoices(mob/user, tabchoice)
	...
	if(current_tab == 0)
		// The classic Character tab is fully replaced by the TGUI character
		// sheet (see character_sheet_ui.dm) — it covers everything this tab
		// used to (identity, vices, colors, voice/bark, flavor & OOC text,
		// gallery, loadout, plus the Hub links for Change Character, Villain
		// Selection, TAT Build, Lore Primer, Changelog, PQ, Triumphs, and Age
		// Verification), so there's nothing left to render here.
		dreamvalley_open_character_sheet_ui(user)
		return
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	if(href_list["preference"] == "dreamvalley_tat")
		...
		return
	if(href_list["preference"] == "dreamvalley_character_sheet")
		dreamvalley_open_character_sheet_ui(user)
		return
	...
	if(href_list["preference"] == "job")
		...
		if(!user.check_agevet())
			to_chat(usr, span_info("- You are a whitelisted player with full access to the server's features. If you'd also like to show others that you've been <b>AGE-VERIFIED</b> with a censored ID, you can open a ticket in our <b>#vet-here</b> channel. If you are already verified on Discord, but not in-game, ahelp. Note that this is a purely optional process, and - besides awarding a special header for your flavortext - doesn't affect you in any other way."))
		else
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("ghostaccs")
					var/new_ghost_accs = alert(user, "Do you want your ghost to show full accessories where possible, hide accessories but still use the directional sprites where possible, or also ignore the directions and stick to the default sprites?",,GHOST_ACCS_FULL_NAME, GHOST_ACCS_DIR_NAME, GHOST_ACCS_NONE_NAME)
				...
				if("ghostothers")
					var/new_ghost_others = alert(user, "Do you want the ghosts of others to show up as their own setting, as their default sprites or always as the default white ghost?",,GHOST_OTHERS_THEIR_SETTING_NAME, GHOST_OTHERS_DEFAULT_SPRITE_NAME, GHOST_OTHERS_SIMPLE_NAME)
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if ("statpack")
					...
					for (var/path as anything in GLOB.statpacks)
						...
						if (!statpack.name)
							continue
						// Fated/Virtuous are retained backend datums for old character
						// saves, but their virtue UI has been superseded by TAT traits.
						if(istype(statpack, /datum/statpack/wildcard/fated) || istype(statpack, /datum/statpack/wildcard/virtuous))
							continue
				...
				if("virtue")
					...
					for (var/path as anything in GLOB.virtues)
						...
						if(V.unlisted)
							continue
						if (istype(V, /datum/virtue/heretic) && !istype(selected_patron, /datum/patron/unveiled))
							continue
