...
GLOBAL_LIST_INIT(praecursor_pool, list(
...
/obj/structure/ascendant_altar/examine(mob/user)
	...
	var/obj/item/next_artefact = LAZYACCESS(GLOB.praecursor_pool, 1)
	var/obj/item/next_capstone = LAZYACCESS(GLOB.praecursor_pool, 1)
	if(next_artefact)
...
/obj/structure/ascendant_altar/proc/consume_artefact(obj/item/I, mob/living/user)
	var/next_artefact = LAZYACCESS(GLOB.praecursor_pool, 1)
	if(!next_artefact)
		return FALSE
	...
	GLOB.praecursor_pool.Cut(1, 2) // remove the first item
	qdel(I)
