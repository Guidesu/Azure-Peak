/mob/living/carbon/simple_animal
	...
	/// Per-zone natural armor (thick hide, scales, etc.)
...
/mob/living/carbon/simple_animal/proc/apply_anatomy_traits()
	var/datum/anatomy/profile = get_anatomy()
	if(profile?.bloodless)
		ADD_TRAIT(src, TRAIT_BLOODLOSS_IMMUNE, TRAIT_GENERIC)

/mob/living/carbon/simple_animal/proc/apply_combat_skill()
...
/mob/living/carbon/simple_animal/proc/get_anatomy()
	if(!anatomy_type)
		return null
	return GLOB.anatomy_profiles[anatomy_type]

/mob/living/carbon/simple_animal/simple_limb_hit(zone)
	var/datum/anatomy/profile = get_anatomy()
...
/mob/living/carbon/simple_animal/hit_zone_name(hit_zone)
	return simple_limb_hit(hit_zone)

/mob/living/carbon/simple_animal/proc/resolve_reachable_zone(zone, obj/item/weapon, mob/living/user)
	var/datum/anatomy/profile = get_anatomy()
	if(!profile)
		return zone
...
/mob/living/carbon/simple_animal/proc/weakpoint_damage_mod(zone)
	var/datum/anatomy/profile = get_anatomy()
	if(!profile)
		return 1
	var/datum/anatomy_zone/hit_zone = profile.get_zone(zone)
...
/mob/living/carbon/simple_animal/get_zone_melee_hit_bonus(zone)
	var/datum/anatomy/profile = get_anatomy()
	if(!profile)
		return 0
	var/datum/anatomy_zone/hit_zone = profile.get_zone(zone)
...
/mob/living/carbon/simple_animal/get_zone_ranged_hit_bonus(zone)
	var/datum/anatomy/profile = get_anatomy()
	if(!profile)
		return 0
	var/datum/anatomy_zone/hit_zone = profile.get_zone(zone)
...
/mob/living/carbon/simple_animal/register_part_damage(zone, damage, mob/living/user, obj/item/weapon, ranged = FALSE, bclass)
...
/mob/living/carbon/simple_animal/proc/warn_unexposed(datum/anatomy_zone/hit_zone, mob/living/user)
...
/mob/living/carbon/simple_animal/proc/announce_newly_exposed(datum/anatomy/profile)
...
/mob/living/carbon/simple_animal/proc/clear_part_damage(zone)
	if(part_damage)
		part_damage[zone] = 0
	LAZYREMOVE(broken_parts, zone)

/mob/living/carbon/simple_animal/proc/reset_part_damage()
...
/mob/living/carbon/simple_animal/proc/topple(duration = 3 SECONDS)
...
/mob/living/carbon/simple_animal/proc/recover_footing()
...
/mob/living/carbon/simple_animal/proc/is_prone()
	return resting || has_wound(/datum/wound/cripple/limb/topple)

/mob/living/carbon/simple_animal/proc/sprite_drawn_prone()
	return LAZYLEN(prone_icon_states) && (icon_state in prone_icon_states)

/mob/living/carbon/simple_animal/apply_damage(damage = 0, damagetype = BRUTE, def_zone = null, blocked = 0, forced = FALSE, spread_damage = FALSE)
	if(def_zone)
		last_hit_part = def_zone
	return ..()

/mob/living/carbon/simple_animal/proc/show_damage_stage()
