...
/mob/living/carbon/human/proc/process_tempo_attack(mob/living/attacker)
	if(isliving(attacker) && attacker != src)
		// Faction flag check — only relevant for carbon vs carbon (player vs player)
		var/attacker_faction = 0
		if(iscarbon(attacker))
			var/mob/living/carbon/C = attacker
			attacker_faction = C.tempo_faction_flag
		if(tempo_faction_flag && attacker_faction && (tempo_faction_flag & attacker_faction))
			return
	...
	switch(id)
		...
		if(TEMPO_TAG_EQUIPTOSS)
			...
			if(has_status_effect(/datum/status_effect/buff/tempo_three))
				...
				return TRUE
		//Spell/miracle power multiplier — tempo empowers bending & miracles
		if(TEMPO_TAG_SPELL_POWER)
			if(has_status_effect(/datum/status_effect/buff/tempo_one))
				return 1.1	//+10% power
			if(has_status_effect(/datum/status_effect/buff/tempo_two))
				return 1.2	//+20% power
			if(has_status_effect(/datum/status_effect/buff/tempo_three))
				return 1.35	//+35% power
			else
				return 1.0
		//Spell/miracle cost multiplier — tempo makes bending cheaper
		if(TEMPO_TAG_SPELL_COST)
			if(has_status_effect(/datum/status_effect/buff/tempo_one))
				return 0.9	//-10% cost
			if(has_status_effect(/datum/status_effect/buff/tempo_two))
				return 0.8	//-20% cost
			if(has_status_effect(/datum/status_effect/buff/tempo_three))
				...
				return 1.0
		//Spell/miracle cooldown reduction (in seconds) — tempo speeds recovery
		if(TEMPO_TAG_SPELL_COOLDOWN)
			if(has_status_effect(/datum/status_effect/buff/tempo_one))
				return 1 SECONDS
			if(has_status_effect(/datum/status_effect/buff/tempo_two))
				return 2 SECONDS
