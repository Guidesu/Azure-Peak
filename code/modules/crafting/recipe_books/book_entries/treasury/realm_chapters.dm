...
/datum/book_entry/treasury_realm/budgets/inner_book_html(mob/user)
	return {"
		<div>
		<h3>Outpost Treasury</h3>
		<p>The settlement's actual mammon balance - one shared pool, not a personal purse. Used to pay wages, imports, deposits, and any other expenditure drawn through the Nerve Master (KEEP IT LOCKED!). Replenished by taxes, fines, direct deposit into the Nerve Master, exports, and fulfilling standing orders.</p>

		<h3>Burgher Pledge</h3>
		<p>Not actual coin, but a virtual pool pledged by the Burghers of the Outpost (off map). It refills daily while the Golden Bull stands, scaling with a flat base and the active player count. Does not refill while the Bull is suspended.</p>

		<h3>Alderman's Warrant</h3>
		<p>An Alderman, elected by the City Assembly, gains a Defense Warrant and a Trade Warrant. Neither is a purse - they are spending ceilings against the Outpost Treasury or Burgher Pledge respectively:</p>
		<ul>
			<li><b>Trade Warrant</b> - a daily mammon ceiling against the Outpost Treasury. The Alderman may import and export up to this amount each day; the monetary value of both imports and exports counts against the Warrant. Coin flows to and from the Treasury itself.</li>
			<li><b>Defense Warrant</b> - a daily Pledge ceiling. Defense commissions issued by the Alderman burn Pledge authority up to this cap, just as the Steward's do. The Alderman may not draw the Outpost Treasury for defense, and may not issue Requests.</li>
		</ul>
		<p>Both ceilings refresh at each session's resolution. Unspent authorisation does not carry over.</p>

		<h3>Treasury Authority</h3>
		<p>The following titles share full Treasury authority - they may petition the trade hall, draw emergency loans, commission defense and blockade writs, and stamp contracts levy-exempt with the signet:</p>
		<ul>
			<li>Steward, Clerk, Hand, Marshal, Councillor.</li>
		</ul>
		<p>The Steward is the primary officer; the rest substitute when the Steward is absent, dead, or otherwise occupied. Whoever the settlement currently looks to for leadership (see <i>Charters of the Realm</i>) holds no mechanical claim over the Treasury by virtue of that title alone - it is purely a figurehead role unless they also hold one of the offices above.</p>
		</div>
	"}
...
/datum/book_entry/treasury_realm/defense/inner_book_html(mob/user)
	return {"
		...
		<ul>
			<li><b>Defense Commissions</b> - drawn against the Pledge or the Outpost Treasury, posted to boards or handed to a bearer.</li>
		...
		<p>When the Steward issues a contract, they may mark it Levy Exempt - the bearer pays no Contract Levy on the reward. This costs the Treasury only the foregone levy and is useful for sweetening offers to mercenaries or adventurers.</p>

		<p>The Steward also starts with an infinite-use signet ring. Any holder of Treasury authority (see <i>Budgets, Warrants & Authority</i>) can stamp a contract (click it) to make it Levy Exempt after the fact - useful for retroactive bribes.</p>

		<h3>Region and Reward</h3>
		<p>Defense commissions pay out in proportion to the threat they spawn. Each threat region carries a <b>reward multiplier</b> (surfaced in the commission UI beside the region name): Greenvalley Basin at x0.75, Whispering Grove at x1.0, Sunken Coast at x1.2, Terrorbog / Mount Cull / Underdark at x1.5. A Bounty in Terrorbog costs the same draft as a Bounty in Greenvalley Basin - but the Terrorbog commission pays the bearer roughly twice as much. The Steward can use this to steer adventurers toward regions the Outpost most needs cleared.</p>

		<p><b>Blockade Writs</b> draw the same flat [BLOCKADE_SCROLL_PLEDGE_COST]m draft regardless of region, and the writ's payout rises with the region's reward multiplier at half weight. The waves are at the same strength everywhere. - the waves are the same strength everywhere, so the premium pays for the road. For example: A Kingsfield blockade writ pays out [BLOCKADE_SCROLL_REWARD]m on completion, while a Mount Decapitation writ pays out [round(BLOCKADE_SCROLL_REWARD * (1 + (1.5 - 1) * BLOCKADE_REWARD_MULT_WEIGHT))]m. Difficulty of Blockade Writ are technically stationary - though some regions are inherently more difficult than others due to the composition within. </p>
...
/datum/book_entry/treasury_realm/trade/inner_book_html(mob/user)
	return {"
		<div>
		<p>The Outpost trades with four independent regions, none of them under its rule or anyone else's: Kingsfield, Rosawood, Daftsmarch, Saltwick. Trade and Stockpile interface are accessed through the Nerve Master's "Trade and Stockpile" interface.
		...
		<p>Each stockpiled good has two prices: a <b>buy price</b> (Treasury pays the depositing player) and a <b>sell price</b> (Treasury charges the withdrawing player).</p>
		<p>On <b>Autoprice</b>, prices peg to the good's global pre-blockade reference (base × event multiplier), guaranteeing the Treasury profits at least 1m per transaction:</p>
		<ul>
		...
		<p>Where <code>import_ref = base × event_mod</code> and <code>export_ref = import_ref × [(1 - IMPORT_EXPORT_SPREAD) * 100]%</code>. At small base prices the +/-1m bound dominates; at larger prices the [IMPORT_EXPORT_SPREAD * 100]% bound takes over so the Treasury's skim scales with the price. No regional lookup - regional shortages, blockades, and route congestion only affect imports/exports, not stockpile prices. The Steward intervenes manually if the situation calls for it.</p>
		<ul>
			...
			<li>Manual-priced entries are skipped by the Treasury's autoexport sweep.</li>
			<li>The Market Scroll surfaces a per-good <b>arbitrage margin</b> column (sell - buy, times current stock) and an aggregate "Treasury spread on held stockpile" total at the top.</li>
		</ul>
		...
		<p>Each stockpile entry has a per-day <b>surplus floor</b>: <code>floor = limit x threshold</code>. Stock above the floor is surplus, and the Treasury's daily auto-export sweep clears that surplus to the highest-paying region, capped at that region's remaining daily demand. The threshold defaults to 60% and is set globally - lower it to make the Treasury more aggressive about turning hoarded stock into mammon, raise it to keep more stock on hand for citizens and standing orders.</p>
		<p>The Steward may also fire the sweep on demand from the Market Scroll's <b>Export Surplus</b> button (or the per-category equivalent). Once a region's daily demand is saturated, no further units can be exported there until the day rolls over - so spamming the button has no effect beyond the first useful click. <b>Manual-priced entries are skipped</b>: those are the Steward's territory, and the Treasury will not auto-route stock the Steward has hand-priced. Hand-export them per-row from the same scroll.</p>

		<h3>Imports and the Stockpile</h3>
		<p>Regional imports enter the Outpost's stockpile and feed standing orders and the city's economy at large. The Steward may set a <b>purchase floor</b>: imports are refused when they would drop the Treasury below it.</p>
...
/datum/book_entry/treasury_realm/auto_import/inner_book_html(mob/user)
	return {"
		<div>
		<p>The Treasury may auto-import essential goods each dawn, sparing the Steward from manually re-importing the same basics every day. Goods stay on the list until removed.</p>
		...
		<ul>
			...
			<li>Otherwise, the Treasury buys [AUTO_IMPORT_BATCH] units from the cheapest producing region.</li>
			<li>The import is skipped if any unit would cost more than [AUTO_IMPORT_MAX_PRICE_MULT]x the good's base price. Shortages can push prices past this cap; standing import stops rather than draining the Treasury.</li>
			<li>The import is skipped if it would drop the Outpost Treasury below the Steward's <b>purse floor</b> (default [AUTO_IMPORT_PURSE_FLOOR_DEFAULT]m, adjustable from the tab).</li>
		</ul>
		...
		<p>Standing imports draw from the Outpost Treasury only. They are not part of any Alderman warrant.</p>
		</div>
	"}
...
<div>
		...
		<h3>Fulfillment</h3>
		<p>Stockpile orders: deposit goods, confirm at the Nerve Master, payout minted to the Outpost Treasury. Warehouse orders: settled automatically on sweep. The Treasury is paid directly and it is up to the Steward to decides who and how much to remit from their profit.</p>
		...
		<h3>Limits</h3>
		<p>Max [STANDING_ORDERS_MAX_PER_REGION] orders per region. Max [STANDING_ORDERS_POOL_CAP] orders across the Outpost. Blockaded regions can hold orders but cannot be delivered to.</p>
		</div>
	"}
...
/datum/book_entry/treasury_realm/insolvent/inner_book_html(mob/user)
	return {"
		<div>
		<p>The Outpost becomes insolvent if it fails to meet payroll from the Outpost Treasury at dawn. Suspending payrolls by revoking Charters is possible but angers the garrison and retinue (the largest mandatory outlays).</p>

		<p>Insolvency triggers in stages: first an interest-free advance, then an optional emergency loan, and finally sequestration if the Outpost fails again.</p>

		<h3>First Failure - Arrears</h3>
		<p>If the Outpost Treasury cannot meet the day's wages, an advance of <b>at least [TREASURY_ARREARS_LOAN]m, up to the actual shortfall</b>, is issued without interest. Wages pay normally for the day. The advance is registered as <b>arrears</b>; until the debt is settled, every coin of inflow into the Outpost Treasury is skimmed against it before reaching the balance.</p>
		...
		<p>Before Day [ATC_LOAN_CLOSED_DAY], the Steward may draw a one-time loan at the Guilds clerk for <b>[ATC_LOAN_MIN_AMOUNT]m to [ATC_LOAN_MAX_AMOUNT]m</b>. The principal is paid into the Outpost Treasury immediately. Interest is <b>[round(ATC_LOAN_INTEREST_RATE * 100)]%</b>: a [ATC_LOAN_MAX_AMOUNT]m draw registers as [round(ATC_LOAN_MAX_AMOUNT * (1 + ATC_LOAN_INTEREST_RATE))]m of debt, repaid silently from skimmed inflow. No second loan may be drawn until the first is settled.</p>

		<p>Drawing the loan is announced and <b>forfeits the arrears grace</b>: missing payroll while the loan is outstanding - by even one mammon - sends the Outpost directly to sequestration without the arrears step.</p>

		<p>From Day [ATC_LOAN_CLOSED_DAY] onward, no further loans may be drawn.</p>

		<h3>Second Failure - Sequestration</h3>
		<p>If the Outpost misses payroll on a second consecutive dawn (or once with an outstanding loan), it enters <b>sequestration</b>:</p>
		<ul>
			<li><b>Outpost Treasury is reset to [BANKRUPTCY_OPERATING_FLOOR]m</b>. Anything above the floor is forfeit; anything below is topped up.</li>
			<li><b>A debt of [BANKRUPTCY_DEBT_FLAT]m</b> is registered on top of any existing arrears or loan debt.</li>
			<li><b>All Treasury-funded salaries are suspended</b> until sequestration lifts.</li>
		...
		<p>The skim continues during sequestration, but the Outpost Treasury may refill up to the [BANKRUPTCY_OPERATING_FLOOR]m operating floor from inflow so trade keeps running. Anything above the floor goes to debt.</p>

		<h3>What the Steward Still Has</h3>
		<ul>
			<li><b>Tax authority</b>: contract levy, headeater levy, import tariff, export duty - all may be set up to the cap, except on burghers.</li>
		...
		<p>When the debt reaches zero, sequestration lifts. Salaries resume the next day. The Outpost Treasury is seeded with <b>[BANKRUPTCY_RECOVERY_RESET]m</b>.</p>

		<p>The Steward may restore <b>up to [BANKRUPTCY_CONCESSION_PICKS] of the suspended Charters immediately</b>, ignoring the cooldown. Charters not picked must wait the standard [DECREE_COOLDOWN / 600]-minute revision cooldown.</p>
		...
		<p>The Outpost may enter sequestration multiple times in the same round. There is no protection against repeat failure - each declaration adds fresh debt.</p>
		</div>
	"}
...
/datum/book_entry/treasury_realm/banditry/inner_book_html(mob/user)
	return {"
		<div>
		<p>Regions classified as <b>Dangerous</b> or <b>Bleak</b> drain the Outpost Treasury each dawn.</p>
		...
		<p>Banditry alone will not reduce the Outpost Treasury below <b>[BANDITRY_DEBT_FLOOR]m</b>. Anything beyond that becomes <b>banditry debt</b> - an accruing arrears that skims every coin of treasury inflow (stockpile earnings, taxes, levies, fines, loan repayments) until paid.</p>
