...
GLOBAL_LIST_EMPTY(volkovoi_cullings)
...
/datum/culling_duel/proc/process_win(mob/living/winner, mob/living/loser)
	...
	to_chat(winner, span_danger("THE WINTER-FATHER SMILES UPON ME. Victory is mine, and with it comes liberated strength of the conquered!"))
...
/datum/round_event_control/volkovoi_culling
	...
	max_occurrences = 1
	min_players = 35
	allowed_storytellers = list(/datum/storyteller/volkovoi)

/datum/round_event_control/volkovoi_culling/canSpawnEvent(players_amt, gamemode, fake_check)
	. = ..()
	if(!.)
		return FALSE
	if(GLOB.patron_follower_counts["Volkovoi"] < 3)
		return FALSE

/datum/round_event/volkovoi_culling/start()
	var/list/contenders = list()
	for(var/mob/living/carbon/human/human_mob in GLOB.player_list)
		if(!istype(human_mob) || human_mob.stat == DEAD || !human_mob.client)
			continue

		if(!human_mob.patron || !istype(human_mob.patron, /datum/patron/oldkin/volkovoi))
			continue
	...
	for(var/i in 1 to max_pairs)
		...
		GLOB.volkovoi_cullings += new_duel
