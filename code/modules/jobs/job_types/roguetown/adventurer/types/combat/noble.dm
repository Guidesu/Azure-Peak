/datum/advclass/noble
	name = "Aristocrat"
	tutorial = "You are a traveling noble visiting the lands surrounding the outpost. With wealth, come the poor, ready to pilfer you of your hard earned (inherited) coin, so tread lightly unless you want to meet a grizzly end."
...
/datum/outfit/job/roguetown/adventurer/noble/pre_equip(mob/living/carbon/human/H)
	...
	if(H.mind)
		var/clothing = list("Classic - Nowhere Significant", "The Outpost", "Grenzelhoft", "Otava", "Aavnr", "Ranesheni", "Gronn", "Etrusca", "Naledi", "Kazengun")
		...
		switch(clothing_choice)
			...
			if("The Outpost")
				...
				//No unique language cause true locals know the tongue. Keeps the idea of foreign marrages, same courts
			...
			if("Naledi") //100% Gender neutral fit, just like the warscholars and refugees
				...
				neck = /obj/item/clothing/neck/roguetown/psicross/g //Not warded or anything, just raw vaeltic status vs ring
...
/datum/advclass/noble/knighte
	name = "Knight Errant"
	tutorial = "You are a knight from a distant land, a scion of a noble house visiting the outpost for one reason or another."
...
/datum/outfit/job/roguetown/adventurer/knighte/pre_equip(mob/living/carbon/human/H)
	..()
	if(H.mind)
		to_chat(H, span_warning("You are a knight from a distant land, a scion of a noble house visiting the outpost for one reason or another."))
		var/helmets = list(
