...
/datum/action/cooldown/spell/auxentius/battle/tug/cast(atom/cast_on)
	. = ..()
...
/datum/action/cooldown/spell/auxentius/battle/provocation
	name = "Provocation"
...
/datum/action/cooldown/spell/auxentius/battle/provocation/cast(atom/cast_on)
	...
	for(var/mob/living/mob in view(dist, get_turf(user)))
		if(!mob.mind && mob.ai_controller)
			...
			checkgate = TRUE
	if(checkgate == TRUE)
		user.apply_status_effect(/datum/status_effect/buff/auxentius_provocation, skill)
	return TRUE
...
/datum/status_effect/buff/auxentius_provocation
	id = "auxentius_provocation"
	alert_type = /atom/movable/screen/alert/status_effect/buff/auxentius_provocation
	duration = 15 SECONDS

/datum/status_effect/buff/auxentius_provocation/on_creation(mob/living/new_owner, assocskill)
	var/con_bonus = 0
	if(assocskill)
		if(assocskill == 1)
			duration = duration
...
/datum/action/cooldown/spell/auxentius/battle/strikeoraegis
	...
	var/chosen_spell
	var/auxentius_strike = /datum/action/cooldown/spell/auxentius/battle/judgement
	var/auxentius_aegis = /datum/action/cooldown/spell/auxentius/battle/aegis
...
/datum/action/cooldown/spell/auxentius/battle/strikeoraegis/cast(atom/cast_on)
	...
	if(choosingspell == TRUE)
		...
		switch(choice)
			if("Judgement - Strike")
				owner.mind?.AddSpell(new auxentius_strike, owner)
				owner.mind?.RemoveSpell(src.type)
			if("Aegis - Shield")
				owner.mind?.AddSpell(new auxentius_aegis, owner)
				owner.mind?.RemoveSpell(src.type)
			else
				return FALSE
...
/datum/action/cooldown/spell/auxentius/battle/judgement
	name = "Judgement"
...
/datum/action/cooldown/spell/auxentius/battle/judgement/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/user = owner
	if(!isliving(user))
		return FALSE
...
/datum/action/cooldown/spell/auxentius/battle/aegis
	...
	var/obj/item/rogueweapon/shield/auxentius_aegis/conjured_shield

/datum/action/cooldown/spell/auxentius/battle/aegis/cast(atom/cast_on)
	. = ..()
	...
	var/obj/item/rogueweapon/shield/auxentius_aegis/S = new(H.drop_location())
	S.linked_spell = src
...
/obj/item/rogueweapon/shield/auxentius_aegis
	...
	associated_skill = /datum/skill/magic/holy
	var/datum/action/cooldown/spell/auxentius/battle/aegis/linked_spell

/obj/item/rogueweapon/shield/auxentius_aegis/getonmobprop(tag)
	. = ..()
	if(tag)
		switch(tag)
			if("gen")
...
/obj/item/rogueweapon/shield/auxentius_aegis/Destroy()
...
/datum/action/cooldown/spell/auxentius/battle/withstand
	name = "Withstand"
...
/datum/action/cooldown/spell/auxentius/battle/withstand/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/user = owner
	if(!isliving(user))
		return FALSE
...
/datum/action/cooldown/spell/auxentius/battle/challenge/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/user = owner
	if(!istype(user))
		return FALSE

	var/area/rogue/indoors/auxentiusarena/thearena = GLOB.areas_by_type[/area/rogue/indoors/auxentiusarena]
	...
	for(var/obj/structure/fluff/auxentius/challenger/aflag in thearena)
		challengerspawnpoint = get_turf(aflag)
	for(var/obj/structure/fluff/auxentius/challenged/bflag in thearena)
	...
	addtimer(CALLBACK(thearena,TYPE_PROC_REF(/area/rogue/indoors/auxentiusarena, cleanthearena), storedchallengedturf), 3 MINUTES) // shunt all items from the arena out onto the challenged spot.
...
/obj/structure/fluff/auxentius/proc/spawnprotection()
...
/datum/action/cooldown/spell/auxentius/battle/persistence
	name = "Persistence"
...
/datum/action/cooldown/spell/auxentius/battle/persistence/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/H = owner
	if(!istype(H))
		return FALSE
...
/datum/action/cooldown/spell/auxentius/battle/persistence/proc/restore_modifiers(datum/physiology/physiology)
	if(!physiology)
		return
...
/datum/action/cooldown/spell/auxentius/battle/battlecry/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/H = owner
	if(!istype(H))
		return FALSE
