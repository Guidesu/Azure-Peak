/datum/action/cooldown/spell/auxentius
...
/datum/action/cooldown/spell/miracle/ignition/auxentius
	background_icon = 'icons/mob/actions/astratamiracles.dmi'
	button_icon = 'icons/mob/actions/astratamiracles.dmi'
	spell_color = GLOW_COLOR_AUXENTIUS_SUN
...
/datum/action/cooldown/spell/auxentius/auxentian_gaze
	name = "Auxentian Gaze"
...
/datum/action/cooldown/spell/auxentius/auxentian_gaze/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/H = owner
	var/skill_level = H.get_skill_level(associated_skill)
	H.apply_status_effect(/datum/status_effect/buff/auxentian_gaze, skill_level)
	return TRUE

/atom/movable/screen/alert/status_effect/buff/auxentian_gaze
	name = "Auxentian Gaze"
	desc = "He shines through me, illuminating all injustice."
	icon_state = "astrata_gaze"

/datum/status_effect/buff/auxentian_gaze
	id = "auxentiangaze"
	alert_type = /atom/movable/screen/alert/status_effect/buff/auxentian_gaze
	duration = 40 SECONDS
	var/skill_level = 0
	status_type = STATUS_EFFECT_REPLACE

/datum/status_effect/buff/auxentian_gaze/on_creation(mob/living/new_owner, slevel)
	// Only store skill level here
	skill_level = slevel
	.=..()

/datum/status_effect/buff/auxentian_gaze/on_apply()
...
/datum/status_effect/buff/auxentian_gaze/on_remove()
	. = ..()
	if(ishuman(owner))
		var/mob/living/carbon/human/H = owner
...
/datum/action/cooldown/spell/miracle/fortify/auxentius
	background_icon = 'icons/mob/actions/astratamiracles.dmi'
	button_icon = 'icons/mob/actions/astratamiracles.dmi'
...
/datum/action/cooldown/spell/auxentius/miracle_pyre
	name = "Solar Pyre"
...
/datum/action/cooldown/spell/auxentius/miracle_pyre/cast(atom/cast_on)
	. = ..()
...
/obj/machinery/light/rogue/campfire/miracle_pyre
	...
	brightness = 10
	fueluse = 15 MINUTES
	color = GLOW_COLOR_AUXENTIUS_SUN
	bulb_colour = GLOW_COLOR_AUXENTIUS_SUN
	max_integrity = 150
	healing_range = 2
	...
	if(on)
		...
		for(var/mob/living/carbon/human/human in hearers_in_range)
			...
			if(istype(human.patron, /datum/patron/concordat))
				...
				human.add_stress(/datum/stressevent/auxentius_pyre)
...
/datum/status_effect/buff/pyre/tick()
	...
	H.color = GLOW_COLOR_AUXENTIUS_SUN
...
/datum/action/cooldown/spell/auxentius/firecloak
...
/datum/action/cooldown/spell/auxentius/firecloak/cast(atom/cast_on)
	. = ..()
	var/mob/living/carbon/human/H = owner
	if(!istype(H))
		return FALSE
	...
	for(var/mob/living/L in range(1, H))
		L.apply_status_effect(/datum/status_effect/buff/dragonhide/auxentius)
...
/obj/effect/proc_holder/spell/invoked/revive/cast(list/targets, mob/living/user)
	...
	if(alert(target, "They are calling for you. Are you ready?", "Revival", "I need to wake up", "Don't let me go") != "I need to wake up")
		target.visible_message(span_auxentius("Nothing happens. They are not being let go."))
		return FALSE
	...
	target.Jitter(100)
	record_round_statistic(STATS_AUXENTIUS_REVIVALS)
	target.update_body()
	target.visible_message(span_auxentius("[target] is revived by holy light!"), span_green("I awake from the void."))
...
/datum/component/immolation/Initialize(mob/living/partner_mob, mob/living/carbon/caster_mob, var/holy_skill, var/is_auxentius)
...
/obj/effect/proc_holder/spell/invoked/immolation/cast(list/targets, mob/living/user)
	...
	var/is_auxentius = (istype(target.patron, /datum/patron/concordat/auxentius))

	// Apply component
	user.AddComponent(/datum/component/immolation, target, user, holy_skill, is_auxentius)
	target.AddComponent(/datum/component/immolation/partner, target, user, holy_skill, is_auxentius)

	// Visual feedback
	user.visible_message(span_auxentius("Holy flames erupt from [user]'s hands and engulf [target]!"))
	if(!is_auxentius)
