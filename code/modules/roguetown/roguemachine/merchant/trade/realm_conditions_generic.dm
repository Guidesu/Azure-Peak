/datum/realm_condition/aavnr_border_skirmish
	...
	description = "A border skirmish has been occuring in a province near the Kamenrad border. While it hasn't escalated into a full scale war, grain trade is disrupted while livestocks are slaughtered for supplies. Hide sells cheap while iron ingots are driven up in price."
	weight = 10
	affected_realms = list(REALM_KAMENRAD)
...
/datum/realm_condition/aavnr_steppe_drought
	...
	name = "Steppe Drought"
	description = "The terraces are dust. The grain fields have failed and the herds are being slaughtered for what they can yield. The Threshold Council's heralds beg foreign caravans for any sack of grain or oats, while hide is sold off cheap from the cull."
	weight = 8
	affected_realms = list(REALM_KAMENRAD)
	supply_modifiers = list(
...
/datum/realm_condition/aavnr_trade_fair
	id = "aavnr_trade_fair"
	name = "Trade Fair"
	description = "A grand trade fair is being held in the heart of Kamenrad. Merchants from across the realm gather to showcase their finest goods, attracting buyers and sellers alike. Hides become cheaper and unique goods are available in the market."
	weight = 6
	affected_realms = list(REALM_KAMENRAD)
...
/datum/realm_condition/etrusca_civil_war
	...
	name = "Civil War"
	description = "Two noble houses have been locked in a bitter civil war for control of a Via Medullan free city. The conflict has disrupted trade and caused widespread instability, leading to a surge in demand for basic metals and a drop in luxury goods like famed fruits and salt."
	weight = 8
...
/datum/realm_condition/etrusca_gronnic_raid
	...
	name = "Punitive Northern Raid"
	description = "Ceaseless raiding has grown intolerable for the free cities of Via Medulla. Its legendary fleet has been sent to raid the coastal settlements of Ognica and burn them down. Food and fruits grow scarce while demand for wood, hide and iron spike as the war effort ramps up."
	weight = 8
	affected_realms = list(REALM_VIAMEDULLA)
	supply_modifiers = list(
...
/datum/realm_condition/grenzelhoft_mage_purge
	...
	weight = 8
	affected_realms = list(REALM_VERGENMARK)
	cultural_modifiers = list(
...
/datum/realm_condition/grenzelhoft_grain_boom
	...
	name = "Grain Boom"
	description = "A bountiful harvest has led to a surplus of grain and oats in Vergenmark. Prices for these staples have plummeted, while demand for fibers has surged as the population seeks to capitalize on the agricultural abundance and sew and buy new garments."
	weight = 10
	affected_realms = list(REALM_VERGENMARK)
	supply_modifiers = list(
...
/datum/realm_condition/gronn_raid_season
	...
	weight = 12
	affected_realms = list(REALM_OGNICA)
	supply_modifiers = list(
...
/datum/realm_condition/kazengun_rebellion
	...
	weight = 8
	affected_realms = list(REALM_OSTROVIA)
	supply_modifiers = list(
...
/datum/realm_condition/cross_chengtian_uprising
	...
	description = "Rebels have risen against the mainland trade-houses that the Tidesworn Isles depend upon. The Xinyi court mobilizes for a counterattack while the isle suzerains reinforce the mainland armies; the whole confederation strips its markets for war supply."
	weight = 12
	affected_realms = list(REALM_OSTROVIA)
	supply_modifiers = list(
...
/datum/realm_condition/lirvas_famine
	id = "lirvas_famine"
	name = "Famine"
	description = "The Marrow Roads' harvests have failed three seasons running, and the lowest rings starve while their debts mount. The Inner Ring opens its vaults - gems and gold ore sell for whatever foreign grain they can buy. Fibers and oats are demanded in huge measure."
	weight = 10
	affected_realms = list(REALM_VIAMEDULLA)
	supply_modifiers = list(
...
/datum/realm_condition/naledi_sandstorm_season
	...
	description = "The Arisole storms have begun early and run long. Glass and gold dust harvests are choked by sand. The desert cities want cloth and cured leather to mend caravans and cover the sacred images during the blow."
	weight = 10
	affected_realms = list(REALM_OSTROVIA)
	supply_modifiers = list(
...
/datum/realm_condition/cross_continental_war
	...
	description = "A formal war has been declared between Vergenmark and the free cities of Via Medulla. Grain is removed from both realms' export ledgers, steel and iron demand spikes brutally, and the looting markets quietly offer foreign silk and gemstones at discount."
	weight = 6
	cross_realm = TRUE
	affected_realms = list(REALM_VERGENMARK, REALM_VIAMEDULLA)
	supply_modifiers = list(
		list("op" = CONDITION_OP_REMOVE, "good" = TRADE_GOOD_GRAIN),
		list("op" = CONDITION_OP_ADD, "good" = TRADE_GOOD_GEMERALD, "qty_min" = BULK_QTY_TINY_MIN, "qty_max" = BULK_QTY_TINY_MAX, "price_mod" = BULK_PRICE_DEEP_DISCOUNT),
...
/datum/realm_condition/cross_northern_raids
	...
	name = "Northern Raids"
	description = "Ognican longships harry the Vergenmark coast. The raiders return to Volfshaven with loot; the Vergenmark ports rearm at any price and offer ransom gold cheap to recover their kin."
	weight = 8
	cross_realm = TRUE
	affected_realms = list(REALM_OGNICA, REALM_VERGENMARK)
...
/datum/realm_condition/cross_eastern_unrest
	...
	name = "Eastern Unrest"
	description = "Disorder rolls across the Asemai. Silk and tea from the eastern isle-ports arrive thin and dear, while the confederation collectively demands iron and coal to settle the unrest."
	weight = 6
	affected_realms = list(REALM_OSTROVIA)
	supply_modifiers = list(
		list("op" = CONDITION_OP_MODIFY, "good" = TRADE_GOOD_SILK, "price_mod" = CONDITION_PRICE_MODERATE, "qty_mod" = CONDITION_QTY_LOW),
...
/datum/realm_condition/cross_raneshen_drought
	...
	name = "Isles Drought"
	description = "The Tidesworn Isles' harvests have collapsed. The island heartlands ship no rice or garlick this season, and the Marrow Roads' tributary fields wither in tandem. Both realms beg for foreign grain and oats."
	weight = 6
	cross_realm = TRUE
	affected_realms = list(REALM_OSTROVIA, REALM_VIAMEDULLA)
	supply_modifiers = list(
		list("op" = CONDITION_OP_REMOVE, "good" = TRADE_GOOD_RICE),
...
/datum/realm_condition/cross_kamenrad_ostrovia_drought
	id = "cross_kamenrad_ostrovia_drought"
	name = "Southern Drought"
	description = "A scorching season has dried the Kamenrad terraces and the dunes alike. The rice paddies of the Tidesworn Isles have gone to dust, while both the Threshold Council and the isle-ports beg the seafaring realms for grain and oats."
	weight = 6
	cross_realm = TRUE
	affected_realms = list(REALM_KAMENRAD, REALM_OSTROVIA)
	supply_modifiers = list(
		list("op" = CONDITION_OP_REMOVE, "good" = TRADE_GOOD_RICE),
	)
	demand_modifiers = list(
...
/datum/realm_condition/cross_ognica_viamedulla_raids
	id = "cross_ognica_viamedulla_raids"
	name = "Southern Raids"
	description = "Ognican longships have ranged south to the Resting Ocean and struck the coastal free cities of the Marrow Roads. The raiders return with citrus and Via Medullan luxuries in their holds; Gran Zafiro mobilizes the Armadas while Falaises-Rouges weeps for its salt flats."
	weight = 5
	cross_realm = TRUE
	affected_realms = list(REALM_OGNICA, REALM_VIAMEDULLA)
	per_realm_modifiers = list(
		REALM_OGNICA = list(
			"supply" = list(
...
/datum/realm_condition/cross_viamedulla_ostrovia_war
	id = "cross_viamedulla_ostrovia_war"
	name = "Coastal Offensive"
	description = "House Zaragoza has answered an old grievance with steel. The Via Medullan Armadas sail east; the coastal counties of the Tidesworn Isles call for the levies and the dervish houses dim their candles."
	weight = 5
	cross_realm = TRUE
	affected_realms = list(REALM_VIAMEDULLA, REALM_OSTROVIA)
	per_realm_modifiers = list(
		REALM_VIAMEDULLA = list(
			"supply" = list(
				list("op" = CONDITION_OP_MODIFY, "good" = TRADE_GOOD_LEMON, "price_mod" = CONDITION_PRICE_MODERATE, "qty_mod" = CONDITION_QTY_LOW),
...
/datum/realm_condition/cross_ostrovia_viamedulla_war
	id = "cross_ostrovia_viamedulla_war"
	name = "Isles Offensive"
	description = "The harbor-council has called the levies. Isle galleys cross the Resting Ocean to strike the Via Medullan coast - and the Vaqueros ride the hills once again. Gran Zafiro pulls every keel to the chain while the port of Vrdaqnan empties its stores for the campaign."
	weight = 5
	cross_realm = TRUE
	affected_realms = list(REALM_OSTROVIA, REALM_VIAMEDULLA)
	per_realm_modifiers = list(
