/obj/structure/roguemachine/ship_fulfillment
	...
	desc = "A wide crate stamped with the seal of the Stewardry. Goods deposited here are accepted against the demands of foreign vessels in port - the depositor is paid in mammon to their account, less the export duty and the Merchant's middleman cut."
...
/obj/structure/roguemachine/ship_fulfillment/proc/attempt_deposit(obj/item/I, mob/user, message = TRUE, sound = TRUE, list/tally)
	...
	if(I.atc_sealed)
		if(message)
			to_chat(user, span_warning("[I] bears a Stewardry seal - foreign captains will not buy Company stock back."))
		return
