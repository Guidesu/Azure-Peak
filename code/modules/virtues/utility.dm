				...
				if (!recipient.islatejoin)
					...
					if(target_z && (recipient.mind?.assigned_role == "Adventurer"))
						...
						if(length(possible_chairs))
							...
							to_chat(recipient, span_notice("As a resident of the outpost, you find yourself seated at a chair in the local tavern."))
						else
							for(var/area/A in world)
								if(!istype(A, /area/rogue/indoors/town/tavern))
									continue
							...
							if(length(possible_spawns))
								...
								to_chat(recipient, span_notice("As a resident of the outpost, you find yourself in the local tavern."))
