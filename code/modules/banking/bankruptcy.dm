...
/datum/controller/subsystem/treasury/proc/enter_arrears()
	...
	log_fund_entry(new /datum/treasury_entry("mint", null, discretionary_fund, loan_amount, "Arrears advance from [SSticker.faction_name || "the Stewardry"]"))
	priority_announce(
		"The outpost's coffers ran dry. [SSticker.faction_name || "The Stewardry"], by their standing pledge, advance [loan_amount]m at no interest to keep the ledger afloat. Should the purse run dry again on the morrow, the outpost enters sequestration.",
		"THE COMPANY LENDS",
...
/datum/controller/subsystem/treasury/proc/enter_bankruptcy()
	...
	if(discretionary_fund.balance > BANKRUPTCY_OPERATING_FLOOR)
		...
		discretionary_fund.balance = BANKRUPTCY_OPERATING_FLOOR
		log_fund_entry(new /datum/treasury_entry("mint", null, discretionary_fund, topup, "Sequestration: operating reserve from [SSticker.faction_name || "the Stewardry"]"))
	...
	switch(treasury_state)
		if(TREASURY_NORMAL)
			if(atc_loan_arrears_consumed)
				...
				priority_announce(
					"The Crown's debt to [SSticker.faction_name || "the Stewardry"] is settled. The Burghers' grace stands restored.",
					"ATC LOAN SETTLED",
					'sound/misc/royal_decree2.ogg',
					"Captain",
				)
...
/// Properties the faction "seizes" against the Crown's debts on bankruptcy entry.
...
/datum/controller/subsystem/treasury/proc/take_atc_loan(amount, mob/applicant)
	...
	priority_announce(
		"The Crown takes an advance of [amount]m from [SSticker.faction_name || "the Stewardry"] at the customary one-quarter interest, registering a debt of [debt_owed]m. The arrears grace stands forfeit; should the Crown miss its next payroll, the realm enters sequestration without warning.",
		"THE CROWN BORROWS",
		'sound/misc/royal_decree.ogg',
		"Captain",
	)
	log_game("ATC LOAN: [applicant ? key_name(applicant) : "system"] drew [amount]m principal from [SSticker.faction_name || "the Stewardry"]; debt of [debt_owed]m registered")
	return TRUE
