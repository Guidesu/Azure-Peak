...
/mob/living/do_game_over()
	...
	if(isanimal(src))
		var/mob/living/carbon/simple_animal/S = src
		S.toggle_ai(AI_OFF)
	if(ishostile(src))
		var/mob/living/carbon/simple_animal/hostile/H = src
		H.LoseTarget()
...
/datum/controller/subsystem/ticker/proc/declare_completion()
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		...
		if(favor_bonus > 0 && H.ckey && H.job && (H.job == "Merchant" || H.job == "Shophand"))
			...
			to_chat(H, "\n<font color='purple'><b>+[favor_bonus] TRIUMPHS</b> awarded for trade volume earned with [SSticker.faction_name || "the Stewardry"].</font>")
...
/datum/controller/subsystem/ticker/proc/get_end_reason()
	...
	if(!check_for_lord(forced = TRUE))
		end_reason = pick("So concluded another chapter of the story. Another begins shortly.",
						...
						"The people of the outpost prepare to look forward; their actions locked in the impermeable past.")
	...
	if(vampire_werewolf() == "vampire")
		end_reason = pick("None can attest to what truly happened this nite; they can only have faith that they did the right thing.",
						"And so, another legend of the nite has chiseled itself into the outpost's history..",
						"The morning's light shines upon a new week, driving away the darkness that threatened the outpost.. for now.",
						"A blank page is filled; a new canvas presented.",
						"Our actors hang up their masks, and a new cast begins to rehearse.",
						"Thus the week's events have taken place. Eventful or mundane, lyfe continues.",
						"Pawns of gods, preachers of nite, all come together to recite this tale.",
	...
	if(SSmapping.retainer.head_rebel_decree)
		end_reason = "The rebellious peasants have taken control of the outpost, shepherding forth the beginning of a new community!"
