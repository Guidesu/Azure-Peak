...
/proc/chronicle_section_population()
	...
	out += chronicle_kv("Total population", "[GLOB.round_stats[STATS_TOTAL_POPULATION]]")
	out += chronicle_kv("Garrison alive", "[GLOB.round_stats[STATS_ALIVE_GARRISON]]")
	out += chronicle_kv("Clergy alive", "[GLOB.round_stats[STATS_ALIVE_CLERGY]]")
	out += chronicle_kv("Tradesmen alive", "[GLOB.round_stats[STATS_ALIVE_TRADESMEN]]")
	out += chronicle_kv("Nobles alive", "[GLOB.round_stats[STATS_ALIVE_NOBLES]]")
	out += chronicle_kv("Gender (M/F/Other)", "[GLOB.round_stats[STATS_MALE_POPULATION]] / [GLOB.round_stats[STATS_FEMALE_POPULATION]] / [GLOB.round_stats[STATS_OTHER_GENDER]]")
	out += chronicle_kv("Age (Adult/Mid/Elderly)", "[GLOB.round_stats[STATS_ADULT_POPULATION]] / [GLOB.round_stats[STATS_MIDDLEAGED_POPULATION]] / [GLOB.round_stats[STATS_ELDERLY_POPULATION]]")
	out += "\n"
	return jointext(out, "")
...
/proc/chronicle_section_treasury()
	...
	out += chronicle_kv("Starting treasury", "[GLOB.round_stats[STATS_STARTING_TREASURY]]")
	out += chronicle_kv("Taxes collected", "[GLOB.round_stats[STATS_TAXES_COLLECTED]]")
	out += chronicle_kv("  Contract levy", "[GLOB.round_stats[STATS_REVENUE_CONTRACT_LEVY]]")
	out += chronicle_kv("  Headeater levy", "[GLOB.round_stats[STATS_REVENUE_HEADEATER_LEVY]]")
	out += chronicle_kv("  Import tariff", "[GLOB.round_stats[STATS_REVENUE_IMPORT_TARIFF]]")
	out += chronicle_kv("  Export duty", "[GLOB.round_stats[STATS_REVENUE_EXPORT_DUTY]]")
	out += chronicle_kv("Poll tax collected", "[GLOB.round_stats[STATS_POLL_TAX_COLLECTED]]")
	out += chronicle_kv("Taxes evaded", "[GLOB.round_stats[STATS_TAXES_EVADED]]")
	var/exempt_total = GLOB.round_stats[STATS_EXEMPTED_CONTRACT_LEVY] + GLOB.round_stats[STATS_EXEMPTED_HEADEATER_LEVY] + GLOB.round_stats[STATS_EXEMPTED_IMPORT_TARIFF] + GLOB.round_stats[STATS_EXEMPTED_EXPORT_DUTY] + GLOB.round_stats[STATS_EXEMPTED_FINE] + GLOB.round_stats[STATS_EXEMPTED_POLL_TAX]
	out += chronicle_kv("Forgone revenue (exemptions)", "[exempt_total]")
	out += chronicle_kv("Wages paid", "[GLOB.round_stats[STATS_WAGES_PAID]]")
	out += chronicle_kv("Fines income", "[GLOB.round_stats[STATS_FINES_INCOME]]")
	out += chronicle_kv("Banditry losses", "[GLOB.round_stats[STATS_BANDITRY_LOSSES]]")
	out += chronicle_kv("Direct treasury transfers", "[GLOB.round_stats[STATS_DIRECT_TREASURY_TRANSFERS]]")
	out += "\n"
	return jointext(out, "")
...
/proc/chronicle_section_foreign_trade()
	...
	out += chronicle_kv("Trade exported (Real Market)", "[GLOB.round_stats[STATS_TRADE_VALUE_EXPORTED]]")
	out += chronicle_kv("Trade exported (Black Market)", "[GLOB.round_stats[STATS_TRADE_VALUE_EXPORTED_BM]]")
	out += chronicle_kv("Trade imported", "[GLOB.round_stats[STATS_TRADE_VALUE_IMPORTED]]")
	out += chronicle_kv("GOLDFACE imports", "[GLOB.round_stats[STATS_GOLDFACE_VALUE_SPENT]]")
	out += chronicle_kv("SILVERFACE imports", "[GLOB.round_stats[STATS_SILVERFACE_VALUE_SPENT]]")
	out += chronicle_kv("COPPERFACE imports", "[GLOB.round_stats[STATS_COPPERFACE_VALUE_SPENT]]")
	out += chronicle_kv("PURITY imports", "[GLOB.round_stats[STATS_PURITY_VALUE_SPENT]]")
	if(SSmerchant_trade)
	...
	out += chronicle_kv("Peddler revenue", "[GLOB.round_stats[STATS_PEDDLER_REVENUE]]")
	out += "\n"
	return jointext(out, "")
...
/proc/chronicle_section_banking()
	...
	out += chronicle_kv("Mammons circulating", "[GLOB.round_stats[STATS_MAMMONS_HELD]]")
	out += chronicle_kv("Mammons deposited", "[GLOB.round_stats[STATS_MAMMONS_DEPOSITED]]")
	out += chronicle_kv("Mammons withdrawn", "[GLOB.round_stats[STATS_MAMMONS_WITHDRAWN]]")
	out += chronicle_kv("Stockpile exports value", "[GLOB.round_stats[STATS_STOCKPILE_EXPORTS_VALUE]]")
	out += chronicle_kv("Stockpile imports value", "[GLOB.round_stats[STATS_STOCKPILE_IMPORTS_VALUE]]")
	out += chronicle_kv("Stockpile revenue", "[GLOB.round_stats[STATS_STOCKPILE_REVENUE]]")
	out += chronicle_kv("Stockpile expanses (sold to)", "[GLOB.round_stats[STATS_STOCKPILE_EXPANSES]]")
	out += chronicle_kv("Bathmatron vault revenue", "[GLOB.round_stats[STATS_BATHMATRON_VAULT_TOTAL_REVENUE]]")
	out += chronicle_kv("Noble estates revenue", "[GLOB.round_stats[STATS_NOBLE_INCOME_TOTAL]]")
	out += chronicle_kv("Loans issued", "[GLOB.round_stats[STATS_LOANS_ISSUED]]")
	out += chronicle_kv("Loans defaulted", "[GLOB.round_stats[STATS_LOANS_DEFAULTED]]")
	out += "\n"
	return jointext(out, "")
...
/proc/chronicle_section_contracts()
	...
	out += chronicle_kv("Generated (total / pool / rumor / def)", "[GLOB.round_stats[STATS_CONTRACTS_GENERATED]] / [GLOB.round_stats[STATS_CONTRACTS_GENERATED_POOL]] / [GLOB.round_stats[STATS_CONTRACTS_GENERATED_RUMOR]] / [GLOB.round_stats[STATS_CONTRACTS_GENERATED_DEFENSE]]")
	out += chronicle_kv("Taken (total / pool / rumor / def)", "[GLOB.round_stats[STATS_CONTRACTS_TAKEN]] / [GLOB.round_stats[STATS_CONTRACTS_TAKEN_POOL]] / [GLOB.round_stats[STATS_CONTRACTS_TAKEN_RUMOR]] / [GLOB.round_stats[STATS_CONTRACTS_TAKEN_DEFENSE]]")
	out += chronicle_kv("Completed (total / pool / rumor / def)", "[GLOB.round_stats[STATS_CONTRACTS_COMPLETED]] / [GLOB.round_stats[STATS_CONTRACTS_COMPLETED_POOL]] / [GLOB.round_stats[STATS_CONTRACTS_COMPLETED_RUMOR]] / [GLOB.round_stats[STATS_CONTRACTS_COMPLETED_DEFENSE]]")
	out += chronicle_kv("Abandoned / Rerolled", "[GLOB.round_stats[STATS_CONTRACTS_ABANDONED]] / [GLOB.round_stats[STATS_CONTRACTS_REROLLED]]")
	out += chronicle_kv("Mammons paid / taxed / forfeited", "[GLOB.round_stats[STATS_CONTRACT_MAMMONS_PAID]] / [GLOB.round_stats[STATS_CONTRACT_MAMMONS_TAXED]] / [GLOB.round_stats[STATS_CONTRACT_MAMMONS_FORFEITED]]")
	out += "\n"
	return jointext(out, "")
...
/proc/chronicle_section_events()
	...
	out += chronicle_kv("Econ events fired / expired", "[GLOB.round_stats[STATS_ECON_EVENTS_FIRED]] / [GLOB.round_stats[STATS_ECON_EVENTS_EXPIRED]]")
	out += chronicle_kv("Urgent orders spawned", "[GLOB.round_stats[STATS_URGENT_ORDERS_SPAWNED]]")
	out += chronicle_kv("Shortages ended", "[GLOB.round_stats[STATS_SHORTAGES_ENDED]]")
	out += chronicle_kv("Blockades fired / cleared", "[GLOB.round_stats[STATS_BLOCKADES_FIRED]] / [GLOB.round_stats[STATS_BLOCKADES_CLEARED]]")
	out += chronicle_kv("Blockade contracts failed", "[GLOB.round_stats[STATS_BLOCKADE_CONTRACTS_FAILED]]")
	out += chronicle_kv("Blockade rewards paid", "[GLOB.round_stats[STATS_BLOCKADE_REWARDS_PAID]]")
	out += "\n"
	return jointext(out, "")
...
/proc/chronicle_section_favors()
	...
	out += chronicle_kv("Standing orders fulfilled / expired / petitioned", "[GLOB.round_stats[STATS_STANDING_ORDERS_FULFILLED]] / [GLOB.round_stats[STATS_STANDING_ORDERS_EXPIRED]] / [GLOB.round_stats[STATS_STANDING_ORDERS_PETITIONED]]")
	out += chronicle_kv("Petition pledge spent", "[GLOB.round_stats[STATS_PETITION_PLEDGE_SPENT]]")
	out += chronicle_kv("Pledge generated / consumed", "[GLOB.round_stats[STATS_PLEDGE_GENERATED]] / [GLOB.round_stats[STATS_PLEDGE_CONSUMED]]")
