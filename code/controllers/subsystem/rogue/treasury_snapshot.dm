...
/datum/controller/subsystem/treasury/proc/compute_charter_states()
	var/list/out = list()
	for(var/id in list(DECREE_GREAT_WRIT, DECREE_ZENITSTADT_CONCORDAT, DECREE_OTAVAN_ACCORDS, DECREE_GOLDEN_BULL, DECREE_MAGNA_CARTA))
...
/// status values: "debtor" TRAIT_DEBTOR, "low_balance" <50m.
...
/datum/controller/subsystem/treasury/proc/compute_filtered_players(status_filter, search_str, include_details = FALSE)
	var/list/out = list()
