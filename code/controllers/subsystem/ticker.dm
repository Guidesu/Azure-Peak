...
/datum/controller/subsystem/ticker/proc/create_characters()
	for(var/i in GLOB.new_player_list)
		var/mob/dead/new_player/player = i
		if(!player)
			message_admins("THERES A FUCKING NULL IN THE NEW_PLAYER_LIST, REPORT IT TO DEVELOPMENT STAFF NOW!")
			continue
		if(!player.mind)
			message_admins("THERES A MIND LACKING PLAYER IN THE NEW_PLAYER_LIST, REPORT IT TO DEVELOPMENT STAFF NOW!")
			continue
		if(player.ready == PLAYER_READY_TO_PLAY)
...
/datum/controller/subsystem/ticker/proc/equip_characters()
	...
	for(var/mob/dead/new_player/new_player as anything in GLOB.new_player_list)
		...
		if(new_player.client?.prefs?.dreamvalley_tat_join_pending)
			continue
