...
/obj/item/natural/thorn/Crossed(mob/living/L)
	...
	if(istype(L))
		...
		if(L.m_intent == MOVE_INTENT_RUN)
			prob2break = 100
		if(prob(prob2break))
			if(L.m_intent == MOVE_INTENT_RUN || !(HAS_TRAIT(L, TRAIT_NATIVEBORN) || HAS_TRAIT(L, TRAIT_NOPVE) || HAS_TRAIT(L, TRAIT_WOODWALKER) || (HAS_TRAIT(L, TRAIT_BOGWALKER) && istype(get_area(L), /area/rogue/outdoors/bog))))
