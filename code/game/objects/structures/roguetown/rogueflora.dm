...
/datum/component/roguegrass/proc/Crossed(datum/source, atom/movable/AM)
	...
	if(isliving(AM))
		...
		if(L.m_intent == MOVE_INTENT_SNEAK)
			...
			if(L.m_intent == MOVE_INTENT_RUN || !(HAS_TRAIT(L, TRAIT_NATIVEBORN) || HAS_TRAIT(L, TRAIT_NOPVE) || (HAS_TRAIT(L, TRAIT_BOGWALKER) && istype(get_area(L), /area/rogue/outdoors/bog))))
