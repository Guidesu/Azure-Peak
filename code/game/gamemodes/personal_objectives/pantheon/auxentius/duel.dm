/datum/objective/auxentius_duel
	name = "Honor Duels"
	triumph_count = 0
	var/duels_won = 0
	var/duels_required = 2

/datum/objective/auxentius_duel/on_creation()
	. = ..()
	if(owner?.current)
		owner.current.mind.AddSpell(new /obj/effect/proc_holder/spell/targeted/auxentius_challenge)
...
/datum/objective/auxentius_duel/update_explanation_text()
	explanation_text = "Win [duels_required] honor duels against other warriors to prove your might!"

/obj/effect/proc_holder/spell/targeted/auxentius_challenge
...
/obj/effect/proc_holder/spell/targeted/auxentius_challenge/cast(list/targets, mob/living/carbon/human/user)
...
/datum/duel/proc/end_duel(mob/living/winner)
	...
	if(winner.mind)
		var/datum/objective/auxentius_duel/objective = locate() in winner.mind.get_all_objectives()
		if(objective)
