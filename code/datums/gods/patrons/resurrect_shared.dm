...
/// Volkovoi (Old Kin), Baotha -> Hausvette (Old Kin), Zizo -> Aurelian (Unveiled). Type paths under
/// /obj/effect/proc_holder/spell/invoked/resurrect/<oldname> were intentionally NOT renamed (they're referenced
/// by miracle lists as mechanics, not identity), but all in-world structure/flavor references have been updated.
...
/obj/effect/proc_holder/spell/invoked/resurrect/matthios
	...
	required_structure = /obj/structure/fluff/psycross/morwenna
...
/obj/effect/proc_holder/spell/invoked/resurrect/graggar
	...
	required_structure = /obj/structure/fluff/psycross/volkovoi
...
/obj/effect/proc_holder/spell/invoked/resurrect/baotha
	...
	required_structure = /obj/structure/fluff/psycross/hausvette
	req_items = list() // temp. hausvette worshippers dont have a holy symbol. apparently one is being commed so this is just the stopgap.
...
/obj/effect/proc_holder/spell/invoked/resurrect/zizo
	...
	zizo = TRUE
	debuff_type = null
	required_structure = /obj/structure/fluff/psycross/aurelian

/// - MORWENNA (formerly Matthios) - ///
...
/datum/status_effect/debuff/graggar_challenge
	...
	effectedstats = list(
		STATKEY_INT = -10 // Volkovoi values brawn over brain
	)
...
/obj/effect/proc_holder/spell/invoked/resurrect/zizo/cast(list/targets, mob/living/carbon/human/user)
	...
	if(.)
		...
		var/found_aurelian_cross = FALSE

		for(var/atom/A in oview(1, target))
			if(istype(A, /obj/structure/fluff/psycross/aurelian))
				found_aurelian_cross = TRUE
				break

			if(istype(A, /turf))
				var/turf/T = A
				for(var/obj/O in T.contents)
					if(istype(O, /obj/structure/fluff/psycross/aurelian))
						found_aurelian_cross = TRUE
						break

			if(found_aurelian_cross)
				break

		// A proper Aurelian cross stabilizes the rite and prevents undeath complications
		if(found_aurelian_cross)
...
/obj/effect/proc_holder/spell/invoked/resurrect/matthios/cast(list/targets, mob/living/carbon/human/user)
	...
	var/auxentius_cross = FALSE

	for(var/atom/A in oview(1, target))
		if(istype(A, /obj/structure/fluff/psycross/morwenna))
			found_cross = A
			break

		if(istype(A, /obj/structure/fluff/psycross/auxentius))
			found_cross = A
			auxentius_cross = TRUE
			break
		...
		if(istype(A, /turf))
			...
			for(var/obj/O in T.contents)
				if(istype(O, /obj/structure/fluff/psycross/morwenna))
					found_cross = O
					break

				if(istype(O, /obj/structure/fluff/psycross/auxentius))
					found_cross = O
					auxentius_cross = TRUE
					break
	...
	if(found_cross && !auxentius_cross)
	...
	to_chat(user, span_userdanger("Without sacred sanction, the miracle runs wild. Both souls are seized by Her ledger."))
