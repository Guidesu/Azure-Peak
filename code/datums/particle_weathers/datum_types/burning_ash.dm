/particles/weather/ash
	// No bespoke "ember"/"ash"/"ash2" particle states exist on particle.dmi;
	// spark/dot are the closest existing small-glowing-particle shapes.
	icon_state				= list("spark"=2, "dot"=5, "dot"=4)
	...
	transform				= null // directional - so don't make it "3D"
...
/datum/particle_weather/ashstorm
	name = "Ashstorm"
	desc = "Firey ash from a distant volcano drifts down over the land."
	particleEffectType = /particles/weather/ash
	warning_message = span_greenannounce("A thunderous cataclysm thunders in the distance and the skies grow unnaturally dark.")
