...
/datum/wound/proc/on_life()
	...
	if(HAS_TRAIT(owner, TRAIT_VAELTITE) && !passive_healing && !HAS_TRAIT(src, TRAIT_BLACKBLOOD))
...
/datum/wound/proc/on_death()
	...
	if (HAS_TRAIT(owner, TRAIT_VAELTITE) && !passive_healing && !HAS_TRAIT(src, TRAIT_BLACKBLOOD))
		heal_wound(0.6) // vaeltites are supposed to apparently slightly heal wounds whether dead or alive
		if(!istype(src, /datum/wound/slash/incision))
			heal_wound(0.6)
	return TRUE
