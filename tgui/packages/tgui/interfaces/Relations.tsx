...
// tgui/packages/tgui/interfaces/Relations.tsx.
//
// Simplified vs. the original: no rival_count/rival_pref readout (that
// tracked Vanderlin's round-start rival-matchmaking preference, which
// wasn't ported — see relations_subsystem.dm on the DM side), and the
// snapshot type only carries the fields DreamValley's
// /datum/relation/refresh_snapshot() actually fills in (name/job/species/
// gender/age — no vcolor/job_category/honorary/job_key, which were
// Vanderlin-specific job-title formatting fields this repo doesn't have).
...
export const Relations = () => {
  ...
  const visible = relations
    ...
    .filter((r) => {
      ...
      const s = r.snapshot;
      const haystack = [formatDisplayName(s, r.name), s?.job, s?.species]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
    ...
    .sort((a, b) =>
      formatDisplayName(a.snapshot, a.name).localeCompare(formatDisplayName(b.snapshot, b.name)),
    );
