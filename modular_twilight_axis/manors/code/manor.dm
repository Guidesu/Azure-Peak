...
/datum/manor/proc/send_foreign_estate_income_mail(mob/living/carbon/human/owner, total_profit_money, estate_levy, import_tariff)
	...
	if(!SSroguemachine.hermailermaster)
		if(owner.client)
			to_chat(owner, span_warning("Your foreign income cannot be delivered - the HERMES mail terminal is unavailable."))
		return
	...
	if(patron == /datum/patron/inhumen/matthios)
		P.info += "We are sending you a share of the revenue we received from selling the goods produced over the past day."
	else
		P.info += "We are sending you the funds received from selling the goods produced by your peasants and workers over the past day."
	P.info += "<BR>Net profit: [total_profit_money] mammon."
	if(estate_levy)
		P.info += "<BR>Estate levy: [estate_levy] mammon."
	if(import_tariff)
...
/datum/manor/proc/produce_resources(mob/living/carbon/human/owner, is_dawn = FALSE, is_dusk = FALSE)
	...
	for(var/datum/workstation/workstation in workstations)
		...
		for(var/i = 1; i <= workstation.workers_employed; i++)
			...
			/*if(patron == /datum/patron/inhumen/matthios)
				...
				if(unsold_units > 0)
					total_profit_money += max(process_goods_sold_to_market(stockpile_entry, unsold_units), 1)*/ //Until I make the link to economic regions
	...
	var/message = "This day your estate supplied the Crown: "
	if(is_foreign)
		message = "This day your estate sold on the market: "
	...
	if(total_profit_money)
		...
		if(estate_levy)
			message += ", minus the estate levy of [estate_levy] mammon"
		if(import_tariff)
			message += (estate_levy ? " and " : ", minus ") + "the import tariff of [import_tariff] mammon"
		if(is_foreign)
			message += ". The funds have been sent to you via HERMES mail"
		message += "."
	else
		if(patron == /datum/patron/inhumen/baotha)
			message += "your treasurer seems too engrossed in idle revelry, and the estate's profit did not arrive."
		else if(patron == /datum/patron/inhumen/matthios)
			message += "your comrades decided not to send you mammon this time."
		else
...
/obj/structure/roguemachine/mail/proc/open_manor_panel(mob/user)
	...
	if(!manor)
		qdel(panel)
		to_chat(H, span_warning("This character does not yet have an available estate."))
		return TRUE
