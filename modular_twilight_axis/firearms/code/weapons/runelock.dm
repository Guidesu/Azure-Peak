/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock
	...
	desc = "An extremely deadly weapon. Uses runic magic instead of gunpowder."
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/examine(mob/user)
	...
	if(ishuman(user))
		...
		if(HAS_TRAIT(u, TRAIT_INQUISITION) || (u.STAINT >= 15) || (u.merctype == 10))
			. += span_info("This weapon is equipped with a runic lock — simply cock the hammer to fire, but it can only be loaded with special runed bullets, crafted from black steel or silver.")
			if(cocked)
				if(chambered)
