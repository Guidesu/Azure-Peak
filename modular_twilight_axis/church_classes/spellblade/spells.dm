...
/obj/effect/proc_holder/spell/targeted/spellblade_select_weapon
	...
	desc = "You momentarily immerse yourself in the world of dreams to choose a weapon suited to you."
	clothes_req = FALSE
	range = -1
	include_user = TRUE
...
/obj/effect/proc_holder/spell/invoked/spellblade_summon_weapon/cast(list/targets, mob/user)
	...
	if(user.put_in_hands(spawned_weapon, del_on_fail = TRUE))
		...
		to_chat(usr, span_warningbig("Something prevents me from using this."))
...
/obj/effect/proc_holder/spell/targeted/spellblade_select_weapon/cast(list/targets, mob/user)
	...
	var/chosed_weapon = tgui_input_list(user, "A few of the things kept in the world of dreams.", "WEAPON CHOICE", weapons_of_choice)
	if(chosed_weapon)
