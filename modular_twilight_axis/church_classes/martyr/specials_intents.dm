...
/datum/special_intent/martyr_ravox_charge/process_attack()
	...
	if(!(howner.mobility_flags & MOBILITY_STAND))
		to_chat(howner, span_warning("I need to be on my feet to charge!"))
		return
	if(!click_loc)
		return
...
/datum/special_intent/martyr_ravox_charge/proc/resolve_charge_impact()
	...
	if(!hit_someone)
		howner.visible_message(
			span_warning("[howner] crashes to the ground after a failed charge!"),
			span_warning("I hit no one, fall to the ground, and momentarily lose all combat capability!")
		)
		howner.Knockdown(self_knockdown_dur)
		howner.Stun(self_stun_dur)
