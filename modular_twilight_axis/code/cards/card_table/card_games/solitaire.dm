...
/datum/card_table_session/proc/solitaire_draw(mob/user)
	...
	if(solitaire_variant == CARD_TABLE_SOLITAIRE_SPIDER)
		...
		for(var/list/column in solitaire_tableau)
			if(!column.len)
				to_chat(user, span_warning("In Spider, you cannot deal from the stock while there is an empty column."))
				return FALSE
		for(var/column_index = 1, column_index <= solitaire_tableau.len, column_index++)
		...
		if(solitaire_completed_sets >= 8)
			...
			message = "[card_table_display_name(user)] clears the spider."
		else
			message = "[card_table_display_name(user)] deals a row from the stock."
		return TRUE
	if(solitaire_stock.len)
		...
		card["face_up"] = TRUE
		discard += list(card)
		message = "[card_table_display_name(user)] reveals a card from the stock."
		return TRUE
	if(discard.len)
		...
		discard = list()
		message = "[card_table_display_name(user)] flips the waste back into the stock."
		return TRUE
	return FALSE
...
/datum/card_table_session/proc/solitaire_move(source_type, source_column, source_index, target_type, target_column, target_suit, mob/user)
	...
	if(target_type == "foundation")
		...
		message = "[card_table_display_name(user)] places [card_table_card_label(card)] into the foundation."
	else
		...
		message = "[card_table_display_name(user)] moves [card_table_card_label(card)]."
		solitaire_check_spider_runs()
	...
	if(foundation_cards >= 52)
		stage = CARD_TABLE_STAGE_FINISHED
		message = "[card_table_display_name(user)] completes the solitaire."
	return TRUE
