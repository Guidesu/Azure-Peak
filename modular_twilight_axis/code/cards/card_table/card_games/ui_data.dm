...
/datum/card_table_session/proc/current_rules() as /list
	...
	switch(game_type)
		if(CARD_TABLE_GAME_FOOL)
			rules += "Goal: be the first to get rid of all cards after the deck runs out."
			rules += "The attacker plays a card, the defender beats it with a higher card of the same suit or a trump."
			if(fool_variant == CARD_TABLE_FOOL_THROW_IN || fool_variant == CARD_TABLE_FOOL_THROW_TRANSFER)
				rules += "Etruscan variant: additional cards can be thrown in matching the rank of cards already on the table. In the first bout, a maximum of 5 attacks, after which you can throw in as long as the defender has cards in hand."
			if(fool_variant == CARD_TABLE_FOOL_TRANSFER || fool_variant == CARD_TABLE_FOOL_THROW_TRANSFER)
				rules += "Otavan variant: the defender can pass the turn with a card of the same rank."
		...
		if(CARD_TABLE_GAME_BLACKJACK)
			...
			rules += "Hit takes a card, Stand locks in your hand."
			rules += "The dealer is always one of the players. [dealer_rotation_label()]."
			switch(blackjack_variant)
				if(CARD_TABLE_BLACKJACK_GRON)
					rules += "Gronn: ace counts as 1; jack, queen and king count as 11."
				if(CARD_TABLE_BLACKJACK_VALORIA)
					rules += "Valorian: ace, jack, queen and king count as 10."
				if(CARD_TABLE_BLACKJACK_GRENZELHOFT)
					rules += "Grenzelhoft: ace counts as 10, but two aces give 21; jack, queen and king count as 10."
				if(CARD_TABLE_BLACKJACK_KAZENGUN)
					rules += "Kazengun: ace, jack, queen and king count as 1."
				else
					rules += "Azurian: ace counts as 11, but two aces give 21; jack, queen and king count as 10."
		if(CARD_TABLE_GAME_POKER)
			rules += "Goal: make the best hand."
			rules += "Betting: enter an amount. If the amount is not higher than the current bet, the Bet button checks or calls; if higher, it raises. All-in bets everything, Fold discards your hand."
			switch(poker_variant)
				if(CARD_TABLE_POKER_TEXAS)
					rules += "Raneshen: player has 2 cards, 5 community cards on the table."
				if(CARD_TABLE_POKER_OMAHA)
					rules += "Valorian: player has 4 cards, 5 community cards on the table."
				if(CARD_TABLE_POKER_STUD)
					rules += "Giza: player receives 5 personal cards with no exchange."
				else
					rules += "Azurian: player receives 5 cards and may replace one card once."
			rules += "The dealer is always one of the players. [dealer_rotation_label()]."
			if(poker_uses_community_cards())
				rules += "Raneshen, Valorian and Giza: community cards are revealed one per round, with a betting round after each card."
			else
				rules += "Azurian: no community cards on the table, only player hands are compared."
		if(CARD_TABLE_GAME_SOLITAIRE)
			rules += "Goal: arrange cards into solitaire foundations."
			if(solitaire_variant == CARD_TABLE_SOLITAIRE_SPIDER)
				rules += "Spider: two decks, ten columns. The stock deals one card to each column."
				rules += "You can move a face-up descending sequence of the same suit. A completed suit from king to ace is removed."
			else
				rules += "Klondike: seven columns, a stock and four foundations by suit from ace to king."
	return rules
