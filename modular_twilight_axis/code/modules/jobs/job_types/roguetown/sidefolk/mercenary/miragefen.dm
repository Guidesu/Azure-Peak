...
/datum/outfit/job/roguetown/mercenary/twilight_miragefen_rogue/pre_equip(mob/living/carbon/human/H) //No hand or paw protection, heh, cats in boots...
	..()
...
/datum/outfit/job/roguetown/mercenary/twilight_miragefen_rogue/choose_loadout(mob/living/carbon/human/H)
	...
	switch(weapon_choice) //Trident for testing purposes, Vladmar said let's see how it plays out, if it's OP as hell, delete this: select and press Backspace.
		if ("Hooksword and Sling")
			...
			H.change_stat(STATKEY_STR, 2) //Results in -1 SPD, +1 STR, since there's -1 STR from above, so the total stat count stays the same.
		if ("Dual Daggers")
			...
			H.put_in_hands(new /obj/item/rogueweapon/huntingknife/idagger/steel/curved_dagger)
		if ("Trident") //Based on the code, 25 force in 1 hand and 20 in 2 hands, with 30 throwforce, let's see how it plays on a dodger. If anything, deleting it takes 10 minutes IRL.
