...
/datum/antagonist/vampire/rockhill_masquerader/proc/expire_clan_choice(mob/living/carbon/human/vampdude)
	if(clan_selected)
		return
	to_chat(vampdude, span_userdanger("I could not decide in time. The Masquerade turns away from me, and the curse leaves my blood..."))
	silent = TRUE
...
/datum/antagonist/vampire/rockhill_masquerader/roundend_report()
	...
	if(triumphs_earned > 0)
		...
		to_chat(owner, span_greentext("My fulfilled ambition has earned me [triumphs_earned] triumph."))
...
/proc/ta_add_rockhill_masquerade_objective(datum/mind/masquerader_mind, datum/objective/rockhill_masquerade/objective)
	...
	to_chat(masquerader_mind.current, span_userdanger("My secret ambitions hide behind the Masquerade."))
...
/datum/objective/rockhill_masquerade/rival_clans
	name = "destroy rival clan leaders"
	explanation_text = "Destroy the leaders of rival clans and make your clan the dominant force in the city."
...
/datum/objective/rockhill_masquerade/inquisition/update_explanation_text()
	...
	explanation_text = "Weaken Otto's influence in Rockhill. Eliminate the following targets: [jointext(target_descriptions, ", ")]."
...
/datum/objective/rockhill_masquerade/royal_conversion/update_explanation_text()
	if(target)
		...
		explanation_text = "Secretly convert [target.name] ([ta_rockhill_role_name_ru(target_job)]) and bring them into my clan."
	else
		explanation_text = "Secretly convert a member of the royal family and bring them into my clan."
...
/datum/objective/rockhill_masquerade/royal_coronation
	name = "coronate the converted ruler"
...
/datum/objective/rockhill_masquerade/royal_coronation/update_explanation_text()
	if(target)
		...
		explanation_text = "Make the converted ruler the true ruler of Rockhill."
...
/datum/objective/rockhill_masquerade/clan_growth/update_explanation_text()
	explanation_text = "Expand my clan to at least [target_clan_size] living vampires."
...
/datum/objective/rockhill_masquerade/methuselah
	name = "decide the Methuselah's fate"
...
/datum/objective/rockhill_masquerade/methuselah/destroy
	name = "destroy the Methuselah"
	explanation_text = "An ancient evil has emerged — a threat to the entire vampire world. It seeks to destroy our clans and plunge the world into war. We must unite against it and destroy the Methuselah."
...
/datum/objective/rockhill_masquerade/methuselah/support
	name = "support the Methuselah"
	explanation_text = "An ancient power has awakened. We should reject the fear of other clans, support the Methuselah's return, and ensure its survival."
