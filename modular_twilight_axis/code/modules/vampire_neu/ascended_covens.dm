...
/mob/living/carbon/human/proc/ta_banish_to_wretch_lair()
	visible_message(span_boldwarning("[src] is enveloped in cursed magic, and their wretched essence dissolves into a portal. These lands can no longer revive the fallen."))
	...
	to_chat(src, span_userdanger("The Lord's blood whispers the way to me: the wretch lair lies in [lair_area?.name || "unknown lands"]."))
	return TRUE
...
/datum/coven_power/potence/ascended_omnipotence/proc/ta_cancel_prepared_strike(show_message = TRUE)
	if(!active)
		return
	if(show_message)
		to_chat(owner, span_notice("I am no longer prepared to use the divine strike."))
