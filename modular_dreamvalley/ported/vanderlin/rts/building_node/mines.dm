// Ported from Vanderlin (OpenKeep): code/datums/rts/building_node/mines.dm
// Direct port. persistant_nodes references work_orders/persistant/mines.dm's
// four singleton mining jobs (ported alongside this file) - on_construction()
// (building_node/building_nodes.dm, Phase 2) instantiates one of each into
// this node's persistant_nodes list the moment the Mine finishes building.
/obj/effect/building_node/mines
	name = "Mine"
	work_template = "mine"

	icon = 'icons/roguetown/items/ore.dmi'
	icon_state = "orecinnabar"

	persistant_nodes = list(
		/datum/persistant_workorder/mine/stones,
		/datum/persistant_workorder/mine/ores,
		/datum/persistant_workorder/mine/coal,
		/datum/persistant_workorder/mine/gem,
	)
