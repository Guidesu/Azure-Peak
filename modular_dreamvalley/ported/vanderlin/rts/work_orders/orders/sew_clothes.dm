// Ported from Vanderlin (OpenKeep): code/datums/rts/work_orders/orders/sew_clothes.dm
// MAT_SILK/MAT_CLOTH renamed to DV_RTS_MAT_SILK/DV_RTS_MAT_CLOTH - see
...
/datum/work_order/sew_clothes/start_working(mob/living/worker_mob)
	if(!tailorshop.use_work_materials(list(DV_RTS_MAT_SILK = 1)))
		worker.controller_mind.pause_task_for(30 SECONDS, workspot)
		tailorshop.add_material_request(src, list(DV_RTS_MAT_SILK = 1), 3)
		return
	. = ..()

/datum/work_order/sew_clothes/finish_work()
	tailorshop.materials_to_store |= DV_RTS_MAT_CLOTH
	tailorshop.materials_to_store[DV_RTS_MAT_CLOTH] += 1
	. = ..()
