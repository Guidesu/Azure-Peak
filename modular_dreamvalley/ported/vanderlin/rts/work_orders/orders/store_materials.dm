// Ported from Vanderlin (OpenKeep): code/datums/rts/work_orders/orders/store_materials.dm
//
// The mirror image of haul_materials.dm: worker walks to a production
// building_node that has accumulated output in its materials_to_store list
// (e.g. a Mines node after its persistent mining jobs finish_work()),
// scoops up to 5 units of each material, then walks to a stockpile and
// deposits them into /datum/stockpile.stored_materials. This is the order
// controller_mob.dm's process() loop (see that file's un-defer below) auto-
// assigns idle workers to whenever constructed_building_nodes has pending
// materials_to_store - the second half of the "mine -> produce -> auto-haul
// -> stockpile" loop this phase proves end to end.
//
// Direct port, same reasoning as haul_materials.dm - no adaptation needed.
...
// ADAPTATION: same start_working() override rationale as haul_materials.dm -
