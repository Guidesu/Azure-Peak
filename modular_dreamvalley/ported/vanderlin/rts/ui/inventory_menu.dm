// Ported from Vanderlin (OpenKeep): code/datums/rts/ui/inventory_menu.dm
//
// PHASE 5 (deferred from Phase 4 - see ui/gear_menu.dm's header comment for
// why). Per-worker equipped-gear panel, opened alongside the existing
// per-worker stats panel (controller_mob.dm's open_inventory_ui(), already
// ported in Phase 4 with a call to this file's open_ui()).
//
// ADAPTATION: upstream's handle_right_click() calls
// `target_node.store_gear(equipped_item, storage_key)` (2 args, with a
// caller-computed storage_key). This port's real store_gear(item, gear_datum)
// (building_node/building_nodes.dm) takes the gear datum instead of a
// pre-computed key and generates/returns its own key - matching
// work_orders/orders/store_gear.dm's same adaptation. attempt_gear_storage()
// below is adjusted to look up the gear datum via
// linked_worker.get_gear_in_slot(slot_type) and pass it directly.
//
// Otherwise a direct port: get_task_display_name()/get_bonus_description()/
// display_gear_stats() all read plain vars/lists on /datum/worker_gear
// (worker_gear.dm, this phase) and TASK_KEY_* defines (_defines.dm, already
// present since Phase 1/this phase's TASK_KEY_QUALITY addition), with no
// adaptation needed.
...
/atom/movable/screen/worker_inventory_slot/proc/handle_left_click(mob/camera/strategy_controller/user)
	if(!linked_worker || !equipped_item)
		to_chat(user, span_warning("No item equipped in [parent_menu.get_slot_display_name(slot_type)] slot."))
		return

	var/datum/worker_gear/gear = linked_worker.get_gear_in_slot(slot_type)
	if(!gear)
		to_chat(user, span_warning("No gear data found for [equipped_item.name]."))
		return
...
/atom/movable/screen/worker_inventory_slot/proc/handle_right_click(mob/camera/strategy_controller/user)
	...
	if(!length(available_nodes))
		to_chat(user, span_warning("No suitable storage buildings found nearby."))
		return
...
// See this file's header ADAPTATION comment: passes the gear datum directly
// to this port's real store_gear(item, gear_datum) instead of a
// caller-computed storage_key.
/atom/movable/screen/worker_inventory_slot/proc/attempt_gear_storage(mob/camera/strategy_controller/user, obj/effect/building_node/target_node)
	...
	var/datum/worker_gear/gear = linked_worker.get_gear_in_slot(slot_type)
	if(!gear)
		to_chat(user, span_warning("Failed to retrieve gear data."))
		return

	linked_worker.worker_gear[slot_type] = null

	target_node.store_gear(equipped_item, gear)
	parent_menu.update_display()

	to_chat(user, span_notice("Stored [equipped_item.name] in [target_node.name]."))

	if(!target_node.Adjacent(linked_worker.worker))
		create_storage_task(user, target_node, gear)

/atom/movable/screen/worker_inventory_slot/proc/create_storage_task(mob/camera/strategy_controller/user, obj/effect/building_node/target_node, datum/worker_gear/gear)
	linked_worker.set_current_task(/datum/work_order/store_gear, target_node, gear)
