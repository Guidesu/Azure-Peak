// Ported from Vanderlin (OpenKeep) code/datums/relations/_menu.dm.
//
// Simplified vs. the original: no rival_count/rival_pref block (that was
// tied to Vanderlin's rival_count preference + round-start matchmaking,
// neither of which exist here — see relations_subsystem.dm), and job
// snapshot fields are trimmed to what /datum/relation/refresh_snapshot()
// in _base.dm actually captures (name/job/species/gender/age; no
// vcolor/job_category/honorary, since those come from Vanderlin-specific
// job title formatting this repo doesn't have).
...
/datum/tgui_relations/ui_data(mob/user)
	...
	for(var/rel_index in 1 to length(mind.relations))
		...
		for(var/history_index in 1 to length(R.relation_history))
			var/datum/relation_history/G = R.relation_history[history_index]
			var/is_gossip = istype(G, /datum/relation_history/gossip)
	...
	switch(action)
		if("say_gossip")
			...
			if(!R || !R.relation_history)
				return FALSE
			var/datum/relation_history/gossip/matched_gossip = R.relation_history[history_index]
			if(!matched_gossip || !istype(matched_gossip, /datum/relation_history/gossip))
				return FALSE
