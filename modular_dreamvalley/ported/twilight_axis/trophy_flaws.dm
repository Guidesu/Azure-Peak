// Ported from Twilight-Axis: two lore-flavored vices gated on a required
// player-quality threshold and job whitelist. Below the threshold (or wrong
// job), they silently fall back to a random flaw instead, matching the
// source behavior exactly.

/datum/charflaw/gefheretic
	name = "Heretic"
	desc = "It doesn't matter where I am running from. From the sands of the south, where Noc holds the dominion, from the north, where shamans offer sacrifices to the spirits, or from the wealthy islands, where Auxentius rules the church. On my face is the mark of the Heretic, inflicted by a vile anointed of God."
	var/required_pq = 15
...
/datum/charflaw/lawless/apply_post_equipment(mob/living/carbon/human/user)
	...
	var/list/states = list(
		"The Outpost Justiciary" = GLOB.outlawed_players,
