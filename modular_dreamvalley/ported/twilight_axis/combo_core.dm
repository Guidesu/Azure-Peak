// Ported from Twilight-Axis: a generic input-history/pattern-matching combo
// engine (attach as a component, define rules as skill-id sequences, get a
// callback when a sequence matches). This is infrastructure only — the two
// concrete combos built on it in Twilight-Axis (a katana "Ronin" class and a
// "Soundbreaker" projectile mechanic) are full class implementations tied to
// spells/weapons/jobs that don't exist here, and were intentionally left out.
// Nothing currently calls into this component; it's available for future
// combat-combo work to build on top of, same as the source intended other
// classes to.
...
// but still shared by future class-specific combo styles.
