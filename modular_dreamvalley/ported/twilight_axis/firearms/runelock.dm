...
// player-facing text translated from Russian into English. The runelock
// pistol is a separate lock mechanism from the matchlock/wheellock/fuse
// twilight_firearm line — it uses runic magic instead of gunpowder, fires
// off the base ballistic magazine system (single-shot internal mag), and is
// gated to Inquisition/high-Intelligence/old-god-patron characters both in
// its equipped() burn-your-hand check and its examine() flavor text.

/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock
	name = "runelock pistol"
	desc = "An extremely lethal weapon. Uses runic magic instead of gunpowder."
	icon = 'modular_dreamvalley/icons/twilight_firearms/32.dmi'
	icon_state = "pistol2"
	...
	fire_sound = 'modular_dreamvalley/sound/twilight_firearms/musketfire2.ogg'
		...
		if(ishuman(user))
			...
			if(HAS_TRAIT(H, TRAIT_INQUISITION) || (H.STAINT >= 15) || (H.merctype == 10))
				...
				if(move_after(H, adj_reload_time SECONDS, target = H))
					...
					playsound(H, 'modular_dreamvalley/sound/twilight_firearms/musketcock.ogg', 100, FALSE)
					cocked = TRUE
			else
				to_chat(H, "<span class='warning'>I have absolutely no idea how to use this!</span>")
		else
			to_chat(user, "<span class='warning'>I have absolutely no idea how to use this!</span>")
	else
		if(alt_grips)
			altgrip(user)
		if(gripped_intents)
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/examine(mob/user)
	...
	if(ishuman(user))
		...
		if(HAS_TRAIT(u, TRAIT_INQUISITION) || (u.STAINT >= 15) || (u.merctype == 10))
			. += span_info("This weapon is fitted with a runic lock — cocking the hammer is all it takes to fire, but it can only be loaded with special runed bullets, forged from black steel or silver.")
			if(cocked)
				if(chambered)
					. += span_bold("Cocked and ready to fire.")
				else
					. += span_bold("The runes are charged with energy, but no ball is loaded.")
			else
				. += span_bold("Unloaded.")
		else
			. += span_info("The lock mechanism fitted to this weapon is unfamiliar to you.")
...
/obj/item/gun/ballistic/revolver/grenadelauncher/twilight_runelock/rifle
	name = "\"Doomsdae\""
	desc = "A relic of a new age, forged for a war that will end history as we know it. Made by Otavan artificers and enchanted by the rune mages of Otava, this runic rifle is a weapon that will crush the legions of darkness at the End of Times. The runes are etched into the weapon's barrel with the blood of heretics who paid for their betrayal of the true faith with their lives."
	icon = 'modular_dreamvalley/icons/twilight_firearms/runelock_rifle.dmi'
	icon_state = "runelock"
