// Ported from Twilight-Axis's firearms/code/jobs/conquistador.dm. Attaches
// automatically to the base Mercenary job via CTAG_MERCENARY category roll.
// All player-facing text translated from Russian.
...
/datum/advclass/mercenary/twilight_conquistador
	...
	classes = list("Tercio" = "Fate placed you in the vanguard, alongside your trusty halberd.",
					"Hidalgo" = "Fate bound your path to gunpowder and to the art of dueling.")
	extra_context = "This class has no starting statline of its own, as all of its stats are assigned after the subclass is chosen. The Tercio subclass has Medium Armor and a halberd; the Hidalgo subclass has Dodge Expert and a pistol with a rapier."
...
/datum/outfit/job/roguetown/mercenary/twilight_conquistador/pre_equip(mob/living/carbon/human/H)
	...
	switch(classchoice)
		if("Tercio (Medium Armor & Halberd)")
			H.set_blindness(0)
			to_chat(H, span_warning("Fate placed you in the vanguard, alongside your trusty halberd."))
			...
			backl = /obj/item/storage/backpack/rogue/satchel/black
			backpack_contents = list(/obj/item/roguekey/mercenary = 1, /obj/item/rogueweapon/huntingknife/idagger/navaja = 1, /obj/item/storage/belt/rogue/pouch/coins/poor = 1, /obj/item/clothing/head/roguetown/duelhat = 1, /obj/item/lockpickring/mundane = 1)
			H.set_blindness(0)
		if("Hidalgo (Light Armor & Rapier + Pistol)")
			to_chat(H, span_warning("Fate bound your path to gunpowder and to the art of dueling."))
