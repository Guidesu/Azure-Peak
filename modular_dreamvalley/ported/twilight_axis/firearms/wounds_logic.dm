...
// No player-facing text to translate — pure crit/wound-roll logic used by
// twilight_firearm bullets on a lucky hit.
	...
	if(NOBLOOD in owner.dna?.species?.species_traits)
		...
		if(!resistance)
			if(zone_precise in earstab_zones)
				...
				var/obj/item/organ/tongue/my_tongue = owner.getorganslot(ORGAN_SLOT_TONGUE)
				if(my_tongue)
