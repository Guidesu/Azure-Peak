...
/obj/effect/proc_holder/spell/self/noctite_fortify/cast_check(skipcharge, mob/user = usr)
	if(GLOB.tod == "day")
		to_chat(usr, "I cannot do this during the day.")
		return
	return ..()
...
/obj/effect/proc_holder/spell/targeted/spellblade_select_weapon
	...
	desc = "You briefly slip into the world of dreams to pick out a weapon suited to you."
	clothes_req = FALSE
	range = -1
	include_user = TRUE
	recharge_time = 0.5 SECONDS
	chargedloop = /datum/looping_sound/invokegen
	action_icon = 'modular_dreamvalley/icons/newmoon_spellblade/ui.dmi'
	action_icon_state = "select_weapon"
	overlay_state = "select_weapon"
	overlay_icon = 'modular_dreamvalley/icons/newmoon_spellblade/ui.dmi'
	invocation_type = "none"
	spell_tier = 2
	cost = 1
...
/obj/effect/proc_holder/spell/invoked/spellblade_summon_weapon
	...
	overlay_icon = 'modular_dreamvalley/icons/newmoon_spellblade/prismatic_weapons64.dmi'
	invocations = list("Noc, grant me a weapon", "Light of Noc, forge me a weapon")
...
/obj/effect/proc_holder/spell/invoked/spellblade_summon_weapon/cast(list/targets, mob/user)
	...
	playsound(get_turf(user), 'modular_dreamvalley/sound/newmoon_spellblade/summon_sfx.ogg', 100, FALSE)
	...
	if(user.put_in_hands(spawned_weapon, del_on_fail = TRUE))
		if(GLOB.tod == "day")
			...
			to_chat(usr, span_warning("I've conjured a weapon and Noc has granted me knowledge of it."))
		...
		to_chat(usr, span_warningbig("Something is preventing me from using this."))
...
/obj/effect/proc_holder/spell/targeted/spellblade_select_weapon/cast(list/targets, mob/user)
	...
	var/chosed_weapon = tgui_input_list(user, "There is little that can be found in the world of dreams.", "WEAPON CHOICE", weapons_of_choice)
	if(chosed_weapon)
