/**
 * Ported from Ratwood-2.0's modular/code/datums/components/intimate_reaction.dm — Stage 1 of the
 * chastity/collar port (components + base chastity item + sexcon2 signal plumbing only; slave collar,
 * chastityplay sex actions, crafting, and sprite/TGUI integration are later stages).
 *
 * Adapted for this repo's sexcon2 architecture:
 *  - Ratwood's COMSIG_CARBON_SEX_ACTION_RECEIVED args were (source, acting_mob, acting_sexcon, action,
 *    receiver_part, giving, arousal_amt, pain_amt, applied_force, applied_speed). This repo's sexcon2 has
 *    no /datum/sex_controller ("sexcon") object — arousal/pain are tracked directly by /datum/component/arousal
 *    on the mob — so acting_sexcon is dropped, and receiver_part becomes acted_sex_part (a new minimal
 *    SEX_PART_* bitmask defined in code/__DEFINES/sex.dm; see get_acted_sex_part() in
 *    code/datums/sexcon2/actions/_base_action.dm for how it's derived).
 *  - Anywhere Ratwood's code called `source.sexcon` / `wearer_sexcon.arousal` / `wearer_sexcon.has_chastity_*()`,
 *    this port queries /datum/component/arousal on the human directly via GetComponent() and, for the
 *    chastity-specific checks, via TRAIT_CHASTITY_* trait checks (has_chastity_penis/vagina/anal/cage below),
 *    since this repo has no sex_controller to host those helper procs on.
 *  - `wearer_sexcon.modular_chastity_content_enabled_for(source)` becomes a direct prefs.chastenable check.
 *  - `wearer_sexcon.last_arousal_increase_time` / `.last_pain` become direct component var reads.
 *  - `wearer_sexcon.is_masochist_in_spiked_chastity()` is reimplemented locally as a proc on the reaction
 *    component (source.has_flaw(/datum/charflaw/addiction/masochist) && HAS_TRAIT(source, TRAIT_CHASTITY_SPIKED)).
 *
 * HOW TO USE — QUICK PATHS:
 * 1. Adding strings to an existing accessory: add entries to the JSON bank under this port's strings/ folder.
 * 2. New accessory with sex-action reactions only: subtype /datum/component/intimate_reaction, override
 *    try_handle_wearer_sex_action_received() and is_valid_wearer_source() (call ..() first).
 * 3. New accessory with movement reactions: also call register_movement_reaction(H)/unregister_movement_reaction(H)
 *    in bind_to_wearer/unbind_from_wearer.
 * 4. Stackable accessories: override dupe_mode to COMPONENT_DUPE_ALLOWED (this repo's equivalent of Ratwood's
 *    COMPONENT_DUPE_ALLOW_ALL — see code/__DEFINES/components.dm) on your subtype.
 * 5. Coverage-aware flavor text: use get_cover_tier_for_zone().
 * 6. Blocking sex actions while worn: use /datum/component/intimate_action_guard.
 */
/datum/component/intimate_reaction
	/// Override to COMPONENT_DUPE_ALLOWED for accessories that can coexist in multiples on the same wearer.
	dupe_mode = COMPONENT_DUPE_UNIQUE
	var/mob/living/carbon/human/wearer = null
	/// Movement cooldown state shared across all subtypes.
...
/datum/component/intimate_reaction/proc/on_wearer_sex_action_received(datum/source, mob/living/carbon/human/acting_mob, datum/sex_action/action, acted_sex_part, giving, arousal_amt, pain_amt, applied_force, applied_speed)
	SIGNAL_HANDLER
	if(source != wearer)
		return FALSE
	return try_handle_wearer_sex_action_received(source, acting_mob, action, acted_sex_part, giving, arousal_amt, pain_amt, applied_force, applied_speed)

/// Stub. Override in subtypes to implement sex-action reactions.
/datum/component/intimate_reaction/proc/try_handle_wearer_sex_action_received(mob/living/carbon/human/source, mob/living/carbon/human/acting_mob, datum/sex_action/action, acted_sex_part, giving, arousal_amt, pain_amt, applied_force, applied_speed)
	return FALSE
...
/// Stub. Override in subtypes to implement movement-based reactions.
 ...
 * Named pick_string_bank to avoid collision with the pick_chastity_string() preprocessor macro.
 */
/datum/component/intimate_reaction/proc/pick_string_bank(filename, string_key, strings_path = CHASTITY_STRINGS_PATH)
	if(!string_key)
		return null
 ...
 * Mirrors get_location_accessible() logic: only real clothing layers count.
 */
...
/datum/component/intimate_action_guard/proc/on_wearer_validate_sex_action(datum/source, mob/living/carbon/human/user, datum/sex_action/action, acted_sex_part, menu_check)
	SIGNAL_HANDLER
	if(source != wearer)
		return FALSE
	return try_validate_wearer_sex_action(source, user, action, acted_sex_part, menu_check)

/// Validates the sex action for the wearer of the chastity device.
/datum/component/intimate_action_guard/proc/try_validate_wearer_sex_action(mob/living/carbon/human/source, mob/living/carbon/human/user, datum/sex_action/action, acted_sex_part, menu_check)
	return FALSE
...
/datum/component/intimate_action_guard/chastity/try_validate_wearer_sex_action(mob/living/carbon/human/source, mob/living/carbon/human/user, datum/sex_action/action, acted_sex_part, menu_check)
	...
	acted_sex_part = acted_sex_part & (SEX_PART_COCK | SEX_PART_CUNT | SEX_PART_ANUS)
	if(!acted_sex_part)
		return FALSE

	if((acted_sex_part & SEX_PART_COCK) && device.has_chastity_penis(source))
		return COMPONENT_SEX_ACTION_BLOCK
	if((acted_sex_part & SEX_PART_CUNT) && device.has_chastity_vagina(source))
		return COMPONENT_SEX_ACTION_BLOCK
	if((acted_sex_part & SEX_PART_ANUS) && device.has_chastity_anal(source))
		return COMPONENT_SEX_ACTION_BLOCK
	return FALSE
...
/// STAGE 1 STUB: Ratwood's /datum/status_effect/facial/internal (tracks retained internal creampie) does not
/// exist anywhere in this repo's status_effects tree — it's not part of this repo's cum/facial system at all.
/// Rather than invent a new status effect as a side effect of this port, this always returns FALSE for now;
/// the vagina-retention arousal/pain banks (chastity_insert_vagina_retention, etc.) are consequently unreachable
/// until a later stage adds the equivalent tracking (or this is wired to whatever this repo uses instead).
/datum/component/intimate_reaction/chastity_receive_flavor/proc/has_retained_creampie(mob/living/carbon/human/source)
	return FALSE

/// Returns TRUE if wearer's spiked pain should be routed through the masochist framing instead of the standard one.
/// Reimplemented locally since this repo has no /datum/sex_controller to host is_masochist_in_spiked_chastity() on.
/datum/component/intimate_reaction/chastity_receive_flavor/proc/is_masochist_in_spiked_chastity(mob/living/carbon/human/source)
	if(!source)
		return FALSE
	return source.has_flaw(/datum/charflaw/addiction/masochist) && HAS_TRAIT(source, TRAIT_CHASTITY_SPIKED)
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/get_receive_flavor_key(mob/living/carbon/human/source, acted_sex_part, datum/sex_action/action, mob/living/carbon/human/acting_mob)
	var/obj/item/chastity/device = source?.chastity_device
	if(!device)
		return null

	var/has_penis_chastity = device.has_chastity_penis(source)
	var/has_vagina_chastity = device.has_chastity_vagina(source)
	var/devout = is_devout_chastity_wearer(source)

	// Anal penetration.
	if(acted_sex_part & SEX_PART_ANUS)
		if(devout)
	...
	// Outercourse: manual stimulation from another person. sexcon2 has no SEX_CATEGORY_HANDS bitmask like
	// Ratwood's old sexcon; the closest equivalent here is the /datum/sex_action/masturbate/other/* subtree,
	// which covers exactly "one human's hand acting on another human's genitals" (jerk/finger someone else off).
	var/is_outercourse = action && istype(action, /datum/sex_action/masturbate/other)
	if(is_outercourse)
		if(devout)
	...
	// Penetrative or unclassified misc — general banks.
	if(devout)
		return "chastity_receive_devout"
	if(has_penis_chastity && has_vagina_chastity)
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/get_arousal_key(mob/living/carbon/human/source)
	...
	if(!device)
		return null
	var/datum/component/arousal/arousal_component = source.GetComponent(/datum/component/arousal)
	if(!arousal_component)
		return null

	// Retention overrides everything — even devout wearers feel that particular hell distinctly.
	if(device.has_chastity_vagina(source) && has_retained_creampie(source))
	...
	if(device.has_chastity_penis(source) && device.has_chastity_vagina(source))
		return "chastity_arousal_intersex"

	// Belt-only wearers.
	if(device.has_chastity_vagina(source))
		return "chastity_arousal_vagina"

	if(!device.has_chastity_cage(source))
		return null
	...
	if(arousal_component.arousal >= ACTIVE_EJAC_THRESHOLD)
		return "chastity_arousal_edge"
	if(arousal_component.arousal >= teasing_threshold)
		return "chastity_arousal_teasing"
	if(arousal_component.arousal >= frustration_threshold)
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/get_arousal_message_chance(mob/living/carbon/human/source, arousal_amt, applied_force, applied_speed)
	var/obj/item/chastity/device = source?.chastity_device
	var/datum/component/arousal/arousal_component = source?.GetComponent(/datum/component/arousal)
	if(!device || !arousal_component)
		return 0

	var/chance = 0
	if(device.has_chastity_vagina(source))
		chance = has_retained_creampie(source) ? 28 : 18
	else if(arousal_component.arousal >= ACTIVE_EJAC_THRESHOLD)
		chance = 30
	else if(arousal_component.arousal >= (ACTIVE_EJAC_THRESHOLD - 30))
		chance = 22
	else if(arousal_component.arousal >= (AROUSAL_HARD_ON_THRESHOLD * 2))
		chance = 16
	else
		chance = 8
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/get_pain_key(mob/living/carbon/human/source, pain_amt)
	...
	// Intersex wearers with both devices spiked get their own bank.
	if(device.has_chastity_penis(source) && device.has_chastity_vagina(source) && pain_amt >= PAIN_MILD_EFFECT)
		if(is_masochist_in_spiked_chastity(source))
			return "chastity_pain_spikes_intersex_masochist"
		return "chastity_pain_spikes_intersex"
	// Vaginal belt spikes take priority over the generic cage banks.
	if(device.has_chastity_vagina(source) && pain_amt >= PAIN_MILD_EFFECT)
		if(is_masochist_in_spiked_chastity(source))
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/get_movement_string_key(mob/living/carbon/human/source)
	...
	var/has_restrictive_chastity = (device.has_chastity_cage(source) || device.has_chastity_anal(source))
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/try_handle_receive_flavor(mob/living/carbon/human/source, acted_sex_part, datum/sex_action/action, mob/living/carbon/human/acting_mob, applied_force, applied_speed)
	if(!source?.client?.prefs?.chastenable)
		return FALSE
	...
	var/string_key = get_receive_flavor_key(source, acted_sex_part, action, acting_mob)
	if(!string_key)
		return FALSE
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/try_handle_arousal_message(mob/living/carbon/human/source, arousal_amt, applied_force, applied_speed)
	var/datum/component/arousal/arousal_component = source?.GetComponent(/datum/component/arousal)
	if(!arousal_component || !source?.client?.prefs?.chastenable)
		return FALSE
	if(source.stat != CONSCIOUS)
		return FALSE
	if(arousal_amt <= 0)
		return FALSE
	if(arousal_component.last_arousal_increase_time != world.time)
		return FALSE
...
/datum/component/intimate_reaction/chastity_receive_flavor/proc/try_handle_pain_message(mob/living/carbon/human/source, pain_amt)
	...
	if(source.stat != CONSCIOUS)
		return FALSE
	if(arousal_component.last_pain != world.time)
		return FALSE
	...
	var/masochist_spiked = !devout_spiked && is_masochist_in_spiked_chastity(source)
	if(devout_spiked)
...
/datum/component/intimate_reaction/chastity_receive_flavor/try_handle_wearer_moved(mob/living/carbon/human/source)
	...
	// All jingle banks produce messages beginning with "'s", concatenated directly onto the name.
...
/datum/component/intimate_reaction/chastity_receive_flavor/try_handle_wearer_sex_action_received(mob/living/carbon/human/source, mob/living/carbon/human/acting_mob, datum/sex_action/action, acted_sex_part, giving, arousal_amt, pain_amt, applied_force, applied_speed)
	...
	if(try_handle_arousal_message(source, arousal_amt, applied_force, applied_speed))
		handled = TRUE
	if(try_handle_receive_flavor(source, acted_sex_part, action, acting_mob, applied_force, applied_speed))
		handled = TRUE
	return handled
